#include "c_p_adic_03_13.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_03_13_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint16_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_03_13 - 1; // Handle 0 as a special case

    uint16_t copy = n << EXPONENT_BITS_P_03_13;
    for (uint8_t i = SIGNIFICAND_BITS_P_03_13 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_32(uint32_t n) {
    ...
    uint32_t copy = n << (2 * EXPONENT_BITS_P_03_13);
    if (copy == 0U) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_03_13 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0U) {
            return i;
        }
...
p0313_t new_p0313(int16_t exp, int16_t sig) {
    return (exp << SIGNIFICAND_BITS_P_03_13) | (sig & SIGNIFICAND_MASK_P_03_13);
}

// exponent part of p-adic 0313
int16_t exp_p0313(p0313_t a) {
    uint16_t exp_a = (a & EXPONENT_MASK_P_03_13) >> SIGNIFICAND_BITS_P_03_13;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_P_03_13 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_03_13 << EXPONENT_BITS_P_03_13; // Fill the left with ones
    }
    return exp_a;
}
...
int16_t sig_p0313(p0313_t a) {
    uint16_t sig_a = a & SIGNIFICAND_MASK_P_03_13;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_P_03_13 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_03_13;
    }
    return sig_a;
}
...
p0313_t add_p0313(p0313_t a, p0313_t b) {
    ...
    int16_t exp_a = (a & EXPONENT_MASK_P_03_13) >> SIGNIFICAND_BITS_P_03_13;
    int16_t exp_b = (b & EXPONENT_MASK_P_03_13) >> SIGNIFICAND_BITS_P_03_13;
    uint16_t sig_a = a & SIGNIFICAND_MASK_P_03_13;
    uint16_t sig_b = b & SIGNIFICAND_MASK_P_03_13;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_03_13 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_03_13;
    ...
    if ((EXPONENT_SIGN_MASK_P_03_13 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_03_13 << EXPONENT_BITS_P_03_13; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p0313(result_exp, result_sig);
}

p0313_t sub_p0313(p0313_t a, p0313_t b) {
    return add_p0313(a, neg_p0313(b));
}

p0313_t neg_p0313(p0313_t b) {
    // Negate b and add
    uint16_t exp_b = b & EXPONENT_MASK_P_03_13;
    uint16_t sig_b = b & SIGNIFICAND_MASK_P_03_13;

    // Flip all bits of significand for two's complement
    if ((1ULL << (SIGNIFICAND_BITS_P_03_13 - 1) & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_03_13;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_03_13;
    }
...
p0313_t mul_p0313(p0313_t a, p0313_t b) {
    uint16_t exp_a = (a & EXPONENT_MASK_P_03_13) >> SIGNIFICAND_BITS_P_03_13;
    uint16_t exp_b = (b & EXPONENT_MASK_P_03_13) >> SIGNIFICAND_BITS_P_03_13;
    uint16_t sig_a = a & SIGNIFICAND_MASK_P_03_13;
    uint16_t sig_b = b & SIGNIFICAND_MASK_P_03_13;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_03_13 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_03_13;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_03_13;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_03_13 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_03_13;
    ...
    // Normalize result
    uint16_t result_sig = (uint16_t)temp_sig & SIGNIFICAND_MASK_P_03_13;
    if (msb > 0) {
...
p0313_t div_p0313(p0313_t a, p0313_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_P_03_13;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_P_03_13;
        }
    }
    ...
    uint16_t mask_r = 1U;
    let((sig_b | 1U) != 0U);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_03_13; i++) {
        uint16_t mask_j = 2U;
        if (i > 0) {
            mask_r = 1U << (i - 1);
        }
...
int com_p0313(p0313_t a, p0313_t b) {
    ...
    int16_t exp_a = (a & EXPONENT_MASK_P_03_13) >> SIGNIFICAND_BITS_P_03_13;
    int16_t exp_b = (b & EXPONENT_MASK_P_03_13) >> SIGNIFICAND_BITS_P_03_13;
    int16_t sig_a = a & SIGNIFICAND_MASK_P_03_13;
    int16_t sig_b = b & SIGNIFICAND_MASK_P_03_13;
