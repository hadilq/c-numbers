#include "c_p_adic_37_27.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_37_27_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint64_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_37_27 - 1; // Handle 0 as a special case

    uint64_t copy = n << EXPONENT_BITS_P_37_27;
    for (uint8_t i = SIGNIFICAND_BITS_P_37_27 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_128(__uint128_t n) {
    ...
    __uint128_t copy = n << (2 * EXPONENT_BITS_P_37_27);
    if (copy == 0ULL) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_37_27 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0ULL) {
            return i;
        }
...
p3727_t new_p3727(int64_t exp, int64_t sig) {
    return (exp << SIGNIFICAND_BITS_P_37_27) | (sig & SIGNIFICAND_MASK_P_37_27);
}

// exponent part of p-adic 3727
int64_t exp_p3727(p3727_t a) {
    uint64_t exp_a = (a & EXPONENT_MASK_P_37_27) >> SIGNIFICAND_BITS_P_37_27;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_P_37_27 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_37_27 << EXPONENT_BITS_P_37_27; // Fill the left with ones
    }
    return exp_a;
}

// significand part of p-adic 3727
int64_t sig_p3727(p3727_t a) {
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_37_27;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_P_37_27 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_37_27;
    }
    return sig_a;
}
...
p3727_t add_p3727(p3727_t a, p3727_t b) {
    ...
    int64_t exp_a = (a & EXPONENT_MASK_P_37_27) >> SIGNIFICAND_BITS_P_37_27;
    int64_t exp_b = (b & EXPONENT_MASK_P_37_27) >> SIGNIFICAND_BITS_P_37_27;
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_37_27;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_37_27;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_37_27 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_37_27;
    ...
    if ((EXPONENT_SIGN_MASK_P_37_27 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_37_27 << EXPONENT_BITS_P_37_27; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p3727(result_exp, result_sig);
}

p3727_t sub_p3727(p3727_t a, p3727_t b) {
    return add_p3727(a, neg_p3727(b));
}

p3727_t neg_p3727(p3727_t b) {
    // Negate b and add
    uint64_t exp_b = b & EXPONENT_MASK_P_37_27;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_37_27;

    // Flip all bits of significand for two's complement
    if ((SIGNIFICAND_SIGN_MASK_P_37_27 & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_37_27;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_37_27;
    }
...
p3727_t mul_p3727(p3727_t a, p3727_t b) {
    uint64_t exp_a = (a & EXPONENT_MASK_P_37_27) >> SIGNIFICAND_BITS_P_37_27;
    uint64_t exp_b = (b & EXPONENT_MASK_P_37_27) >> SIGNIFICAND_BITS_P_37_27;
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_37_27;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_37_27;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_37_27 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_37_27;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_37_27;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_37_27 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_37_27;
    ...
    // Normalize result
    uint64_t result_sig = (uint64_t)temp_sig & SIGNIFICAND_MASK_P_37_27;
    if (msb > 0) {
...
p3727_t div_p3727(p3727_t a, p3727_t b) {
    ...
    if (sig_b == 0ULL) {
        if (posi_a) {
            ...
            return MAX_VALUE_P_37_27;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_P_37_27;
        }
    }
    ...
    let((sig_b | 1ULL) != 0ULL);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_37_27; i++) {
...
int com_p3727(p3727_t a, p3727_t b) {
    ...
    int64_t exp_a = (a & EXPONENT_MASK_P_37_27) >> SIGNIFICAND_BITS_P_37_27;
    int64_t exp_b = (b & EXPONENT_MASK_P_37_27) >> SIGNIFICAND_BITS_P_37_27;
    int64_t sig_a = a & SIGNIFICAND_MASK_P_37_27;
    int64_t sig_b = b & SIGNIFICAND_MASK_P_37_27;
