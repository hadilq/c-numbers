#include "c_p_adic_03_61.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_03_61_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint64_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_03_61 - 1; // Handle 0 as a special case

    uint64_t copy = n << EXPONENT_BITS_P_03_61;
    for (uint8_t i = SIGNIFICAND_BITS_P_03_61 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_128(__uint128_t n) {
    ...
    __uint128_t copy = n << (2 * EXPONENT_BITS_P_03_61);
    if (copy == 0ULL) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_03_61 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0ULL) {
            return i;
        }
...
uint64_t exp_p0361(p0361_t a) {
    ...
    if ((EXPONENT_SIGN_MASK_P_03_61 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_03_61 << EXPONENT_BITS_P_03_61; // Fill the left with ones
    }
    return exp_a;
}
...
uint64_t sig_p0361(p0361_t a) {
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_03_61 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_03_61;
    }
    return sig_a;
}
...
p0361_t add_p0361(p0361_t a, p0361_t b) {
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_03_61 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_03_61;
    ...
    if ((EXPONENT_SIGN_MASK_P_03_61 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_03_61 << EXPONENT_BITS_P_03_61; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p0361(result_exp, result_sig);
}

p0361_t sub_p0361(p0361_t a, p0361_t b) {
    return add_p0361(a, neg_p0361(b));
}

p0361_t neg_p0361(p0361_t b) {
    // Negate b and add
    uint64_t exp_b = b & EXPONENT_MASK_P_03_61;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_03_61;

    // Flip all bits of significand for two's complement
    if ((SIGNIFICAND_SIGN_MASK_P_03_61 & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_03_61;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_03_61;
    }
...
p0361_t mul_p0361(p0361_t a, p0361_t b) {
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_03_61 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_03_61;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_03_61;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_03_61 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_03_61;
    ...
    // Normalize result
    uint64_t result_sig = (uint64_t)temp_sig & SIGNIFICAND_MASK_P_03_61;
    if (msb > 0) {
...
p0361_t div_p0361(p0361_t a, p0361_t b) {
    ...
    let((sig_b | 1ULL) != 0ULL);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_03_61; i++) {
