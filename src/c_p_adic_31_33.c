#include "c_p_adic_31_33.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_31_33_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint64_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_31_33 - 1; // Handle 0 as a special case

    uint64_t copy = n << EXPONENT_BITS_P_31_33;
    for (uint8_t i = SIGNIFICAND_BITS_P_31_33 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_128(__uint128_t n) {
    ...
    __uint128_t copy = n << (2 * EXPONENT_BITS_P_31_33);
    if (copy == 0ULL) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_31_33 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0ULL) {
            return i;
        }
...
p3133_t new_p3133(uint64_t exp, uint64_t sig) {
    return (exp << SIGNIFICAND_BITS_P_31_33) | (sig & SIGNIFICAND_MASK_P_31_33);
}

// exponent part of p-adic 3133
uint64_t exp_p3133(p3133_t a) {
    uint64_t exp_a = (a & EXPONENT_MASK_P_31_33) >> SIGNIFICAND_BITS_P_31_33;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_P_31_33 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_31_33 << EXPONENT_BITS_P_31_33; // Fill the left with ones
    }
    return exp_a;
}

// significand part of p-adic 3133
uint64_t sig_p3133(p3133_t a) {
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_31_33;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_P_31_33 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_31_33;
    }
    return sig_a;
}
...
p3133_t add_p3133(p3133_t a, p3133_t b) {
    ...
    int64_t exp_a = (a & EXPONENT_MASK_P_31_33) >> SIGNIFICAND_BITS_P_31_33;
    int64_t exp_b = (b & EXPONENT_MASK_P_31_33) >> SIGNIFICAND_BITS_P_31_33;
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_31_33;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_31_33;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_31_33 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_31_33;
    ...
    if ((EXPONENT_SIGN_MASK_P_31_33 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_31_33 << EXPONENT_BITS_P_31_33; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p3133(result_exp, result_sig);
}

p3133_t sub_p3133(p3133_t a, p3133_t b) {
    return add_p3133(a, neg_p3133(b));
}

p3133_t neg_p3133(p3133_t b) {
    // Negate b and add
    uint64_t exp_b = b & EXPONENT_MASK_P_31_33;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_31_33;

    // Flip all bits of significand for two's complement
    if ((SIGNIFICAND_SIGN_MASK_P_31_33 & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_31_33;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_31_33;
    }
...
p3133_t mul_p3133(p3133_t a, p3133_t b) {
    uint64_t exp_a = (a & EXPONENT_MASK_P_31_33) >> SIGNIFICAND_BITS_P_31_33;
    uint64_t exp_b = (b & EXPONENT_MASK_P_31_33) >> SIGNIFICAND_BITS_P_31_33;
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_31_33;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_31_33;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_31_33 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_31_33;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_31_33;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_31_33 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_31_33;
    ...
    // Normalize result
    uint64_t result_sig = (uint64_t)temp_sig & SIGNIFICAND_MASK_P_31_33;
    if (msb > 0) {
...
p3133_t div_p3133(p3133_t a, p3133_t b) {
    ...
    if (sig_b == 0ULL) {
        if (posi_a) {
            ...
            return (((1ULL << (EXPONENT_BITS_P_31_33 - 2)) - 1) << SIGNIFICAND_BITS_P_31_33)
                   | ((1ULL << (SIGNIFICAND_BITS_P_31_33 - 2)) - 1);
        } else {
            // Return min value as "-infinity"
            return (((1ULL << (EXPONENT_BITS_P_31_33 - 2)) - 1) << SIGNIFICAND_BITS_P_31_33)
                   | (SIGNIFICAND_SIGN_MASK_P_31_33);
        }
    }
    ...
    let((sig_b | 1ULL) != 0ULL);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_31_33; i++) {
