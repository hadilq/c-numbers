#include "c_p_adic_13_19.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_13_19_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint32_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_13_19 - 1; // Handle 0 as a special case

    uint32_t copy = n << EXPONENT_BITS_P_13_19;
    for (uint8_t i = SIGNIFICAND_BITS_P_13_19 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_64(uint64_t n) {
    ...
    uint64_t copy = n << (2 * EXPONENT_BITS_P_13_19);
    if (copy == 0U) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_13_19 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0U) {
            return i;
        }
...
p1319_t new_p1319(uint32_t exp, uint32_t sig) {
    return (exp << SIGNIFICAND_BITS_P_13_19) | (sig & SIGNIFICAND_MASK_P_13_19);
}

// exponent part of p-adic 1319
uint32_t exp_p1319(p1319_t a) {
    uint32_t exp_a = (a & EXPONENT_MASK_P_13_19) >> SIGNIFICAND_BITS_P_13_19;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_P_13_19 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_13_19 << EXPONENT_BITS_P_13_19; // Fill the left with ones
    }
    return exp_a;
}

// significand part of p-adic 1319
uint32_t sig_p1319(p1319_t a) {
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_13_19;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_P_13_19 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_13_19;
    }
    return sig_a;
}
...
p1319_t add_p1319(p1319_t a, p1319_t b) {
    ...
    int32_t exp_a = (a & EXPONENT_MASK_P_13_19) >> SIGNIFICAND_BITS_P_13_19;
    int32_t exp_b = (b & EXPONENT_MASK_P_13_19) >> SIGNIFICAND_BITS_P_13_19;
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_13_19;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_13_19;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_13_19 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_13_19;
    ...
    if ((EXPONENT_SIGN_MASK_P_13_19 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_13_19 << EXPONENT_BITS_P_13_19; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p1319(result_exp, result_sig);
}

p1319_t sub_p1319(p1319_t a, p1319_t b) {
    return add_p1319(a, neg_p1319(b));
}

p1319_t neg_p1319(p1319_t b) {
    // Negate b and add
    uint32_t exp_b = b & EXPONENT_MASK_P_13_19;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_13_19;

    // Flip all bits of significand for two's complement
    if ((1ULL << (SIGNIFICAND_BITS_P_13_19 - 1) & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_13_19;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_13_19;
    }
...
p1319_t mul_p1319(p1319_t a, p1319_t b) {
    uint32_t exp_a = (a & EXPONENT_MASK_P_13_19) >> SIGNIFICAND_BITS_P_13_19;
    uint32_t exp_b = (b & EXPONENT_MASK_P_13_19) >> SIGNIFICAND_BITS_P_13_19;
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_13_19;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_13_19;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_13_19 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_13_19;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_13_19;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_13_19 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_13_19;
    ...
    // Normalize result
    uint32_t result_sig = (uint32_t)temp_sig & SIGNIFICAND_MASK_P_13_19;
    if (msb > 0) {
...
p1319_t div_p1319(p1319_t a, p1319_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return (((1U << (EXPONENT_BITS_P_13_19 - 2)) - 1) << SIGNIFICAND_BITS_P_13_19)
                   | ((1U << (SIGNIFICAND_BITS_P_13_19 - 2)) - 1);
        } else {
            // Return min value as "-infinity"
            return (((1U << (EXPONENT_BITS_P_13_19 - 2)) - 1) << SIGNIFICAND_BITS_P_13_19)
                   | (SIGNIFICAND_SIGN_MASK_P_13_19);
        }
    }
    ...
    uint32_t mask_r = 1U;
    let((sig_b | 1U) != 0U);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_13_19; i++) {
        uint32_t mask_j = 2U;
        if (i > 0) {
            mask_r = 1U << (i - 1);
        }
