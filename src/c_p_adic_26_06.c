...
static uint8_t get_msb_index(uint32_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_26_06 - 1; // Handle 0 as a special case

    uint32_t copy = n << EXPONENT_BITS_P_26_06;
    for (uint8_t i = SIGNIFICAND_BITS_P_26_06 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_64(uint64_t n) {
    ...
    uint64_t copy = n << (2 * EXPONENT_BITS_P_26_06);
    if (copy == 0U) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_26_06 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0U) {
            return i;
        }
...
p2606_t add_p2606(p2606_t a, p2606_t b) {
    ...
    // Assemble result
    return new_p2606(result_exp, result_sig);
}

p2606_t sub_p2606(p2606_t a, p2606_t b) {
    return add_p2606(a, neg_p2606(b));
}

p2606_t neg_p2606(p2606_t b) {
    // Negate b and add
    uint32_t exp_b = b & EXPONENT_MASK_P_26_06;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_26_06;

    // Flip all bits of significand for two's complement
    if ((1ULL << (SIGNIFICAND_BITS_P_26_06 - 1) & sig_b)) {
        // negative
...
p2606_t mul_p2606(p2606_t a, p2606_t b) {
    ...
    // Normalize result
    uint32_t result_sig = (uint32_t)temp_sig & SIGNIFICAND_MASK_P_26_06;
    if (msb > 0) {
...
p2606_t div_p2606(p2606_t a, p2606_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_P_26_06;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_P_26_06;
        }
    }
    ...
    uint32_t mask_r = 1U;
    let((sig_b | 1U) != 0U);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_26_06; i++) {
        uint32_t mask_j = 2U;
        if (i > 0) {
            mask_r = 1U << (i - 1);
        }
...
int com_p2606(p2606_t a, p2606_t b) {
    ...
    int32_t exp_a = (a & EXPONENT_MASK_P_26_06) >> SIGNIFICAND_BITS_P_26_06;
    int32_t exp_b = (b & EXPONENT_MASK_P_26_06) >> SIGNIFICAND_BITS_P_26_06;
    int32_t sig_a = a & SIGNIFICAND_MASK_P_26_06;
    int32_t sig_b = b & SIGNIFICAND_MASK_P_26_06;
