#include "c_p_adic_09_55.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_09_55_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint64_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_09_55 - 1; // Handle 0 as a special case

    uint64_t copy = n << EXPONENT_BITS_P_09_55;
    for (uint8_t i = SIGNIFICAND_BITS_P_09_55 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_128(__uint128_t n) {
    ...
    __uint128_t copy = n << (2 * EXPONENT_BITS_P_09_55);
    if (copy == 0ULL) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_09_55 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0ULL) {
            return i;
        }
...
p0955_t new_p0955(uint64_t exp, uint64_t sig) {
    return (exp << SIGNIFICAND_BITS_P_09_55) | (sig & SIGNIFICAND_MASK_P_09_55);
}

// exponent part of p-adic 0955
uint64_t exp_p0955(p0955_t a) {
    uint64_t exp_a = (a & EXPONENT_MASK_P_09_55) >> SIGNIFICAND_BITS_P_09_55;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_P_09_55 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_09_55 << EXPONENT_BITS_P_09_55; // Fill the left with ones
    }
    return exp_a;
}

// significand part of p-adic 0955
uint64_t sig_p0955(p0955_t a) {
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_09_55;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_P_09_55 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_09_55;
    }
    return sig_a;
}
...
p0955_t add_p0955(p0955_t a, p0955_t b) {
    ...
    int64_t exp_a = (a & EXPONENT_MASK_P_09_55) >> SIGNIFICAND_BITS_P_09_55;
    int64_t exp_b = (b & EXPONENT_MASK_P_09_55) >> SIGNIFICAND_BITS_P_09_55;
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_09_55;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_09_55;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_09_55 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_09_55;
    ...
    if ((EXPONENT_SIGN_MASK_P_09_55 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_09_55 << EXPONENT_BITS_P_09_55; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p0955(result_exp, result_sig);
}

p0955_t sub_p0955(p0955_t a, p0955_t b) {
    return add_p0955(a, neg_p0955(b));
}

p0955_t neg_p0955(p0955_t b) {
    // Negate b and add
    uint64_t exp_b = b & EXPONENT_MASK_P_09_55;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_09_55;

    // Flip all bits of significand for two's complement
    if ((SIGNIFICAND_SIGN_MASK_P_09_55 & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_09_55;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_09_55;
    }
...
p0955_t mul_p0955(p0955_t a, p0955_t b) {
    uint64_t exp_a = (a & EXPONENT_MASK_P_09_55) >> SIGNIFICAND_BITS_P_09_55;
    uint64_t exp_b = (b & EXPONENT_MASK_P_09_55) >> SIGNIFICAND_BITS_P_09_55;
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_09_55;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_09_55;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_09_55 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_09_55;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_09_55;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_09_55 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_09_55;
    ...
    // Normalize result
    uint64_t result_sig = (uint64_t)temp_sig & SIGNIFICAND_MASK_P_09_55;
    if (msb > 0) {
...
p0955_t div_p0955(p0955_t a, p0955_t b) {
    ...
    if (sig_b == 0ULL) {
        if (posi_a) {
            ...
            return (((1ULL << (EXPONENT_BITS_P_09_55 - 2)) - 1) << SIGNIFICAND_BITS_P_09_55)
                   | ((1ULL << (SIGNIFICAND_BITS_P_09_55 - 2)) - 1);
        } else {
            // Return min value as "-infinity"
            return (((1ULL << (EXPONENT_BITS_P_09_55 - 2)) - 1) << SIGNIFICAND_BITS_P_09_55)
                   | (SIGNIFICAND_SIGN_MASK_P_09_55);
        }
    }
    ...
    let((sig_b | 1ULL) != 0ULL);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_09_55; i++) {
