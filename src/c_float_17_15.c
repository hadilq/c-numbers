...
static uint8_t get_msb_index_positive(uint32_t n) {
    ...
    uint32_t copy = n & SIGNIFICAND_MASK_F_17_15;
    for (uint8_t i = 1; i <= SIGNIFICAND_BITS_F_17_15; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_64(uint64_t n) {
    ...
    uint64_t copy = n & SIGNIFICAND_2_MASK_F_17_15;
    for (uint8_t i = 1; i <= 2 * SIGNIFICAND_BITS_F_17_15; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
f1715_t new_f1715(int32_t exp, int32_t sig) {
    return (exp << SIGNIFICAND_BITS_F_17_15) | (sig & SIGNIFICAND_MASK_F_17_15);
}
...
f1715_t new_f1715_from_float32(_Float32 f) {
    ...
    if (SIGNIFICAND_BITS_F_17_15 > Float32_SIG_1) {
        significand <<= SIGNIFICAND_BITS_F_17_15 - Float32_SIG_1;
    } else {
        significand >>= Float32_SIG_1 - SIGNIFICAND_BITS_F_17_15;
    }
    return new_f1715(exponent, significand);
}
...
f1715_t new_f1715_from_float16(_Float16 f) {
    ...
    if (SIGNIFICAND_BITS_F_17_15 > Float16_SIG_1) {
        significand <<= SIGNIFICAND_BITS_F_17_15 - Float16_SIG_1;
    } else {
        significand >>= Float16_SIG_1 - SIGNIFICAND_BITS_F_17_15;
    }
    return new_f1715(exponent, significand);
}
...
int32_t exp_f1715(f1715_t a) {
    uint32_t exp_a = (a & EXPONENT_MASK_F_17_15) >> SIGNIFICAND_BITS_F_17_15;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_F_17_15 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_F_17_15 << EXPONENT_BITS_F_17_15; // Fill the left with ones
    }
    return exp_a;
}
...
int32_t sig_f1715(f1715_t a) {
    uint32_t sig_a = a & SIGNIFICAND_MASK_F_17_15;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_F_17_15 & sig_a)) {
        sig_a |= EXPONENT_MASK_F_17_15;
    }
    return sig_a;
}
...
f1715_t add_f1715(f1715_t a, f1715_t b) {
    ...
    int32_t exp_a = (a & EXPONENT_MASK_F_17_15) >> SIGNIFICAND_BITS_F_17_15;
    int32_t exp_b = (b & EXPONENT_MASK_F_17_15) >> SIGNIFICAND_BITS_F_17_15;
    uint32_t sig_a = a & SIGNIFICAND_MASK_F_17_15;
    uint32_t sig_b = b & SIGNIFICAND_MASK_F_17_15;
    ...
    if ((SIGNIFICAND_SIGN_MASK_F_17_15 & sig_b)) {
        sig_b |= EXPONENT_MASK_F_17_15;
        posi_b = false;
    ...
    if ((EXPONENT_SIGN_MASK_F_17_15 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_F_17_15 << EXPONENT_BITS_F_17_15; // Fill the left with ones
    }
    ...
    if (exp_a > exp_b) {
        ...
        sig_b >>= diff;
        if (!posi_b) {
            sig_b |= ((1U << diff) - 1) << (BITS_F_17_15 - diff); // Fill the left with ones
            if ((sig_b & SIGNIFICAND_MASK_F_17_15) == SIGNIFICAND_MASK_F_17_15) {
                sig_b = 0;
            }
        }
        ...
        sig_a >>= diff;
        if (!posi_a) {
            sig_a |= ((1U << diff) - 1) << (BITS_F_17_15 - diff); // Fill the left with ones
            if ((sig_a & SIGNIFICAND_MASK_F_17_15) == SIGNIFICAND_MASK_F_17_15) {
                sig_a = 0;
            }
        }
    ...
    if (BITS_SIGN_MASK_F_17_15 & result_sig) {
       posi_result = false;
    }
    ...
    msb -= SIGNIFICAND_BITS_F_17_15 - 1;
    ...
    // Assemble result
    return new_f1715(result_exp, result_sig);
}

f1715_t sub_f1715(f1715_t a, f1715_t b) {
    return add_f1715(a, neg_f1715(b));
}

f1715_t neg_f1715(f1715_t b) {
    // Negate b and add
    uint32_t exp_b = b & EXPONENT_MASK_F_17_15;
    uint32_t sig_b = b & SIGNIFICAND_MASK_F_17_15;

    // Flip all bits of significand for two's complement
    if ((SIGNIFICAND_SIGN_MASK_F_17_15 & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_F_17_15;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_F_17_15;
    }
...
f1715_t mul_f1715(f1715_t a, f1715_t b) {
    uint32_t exp_a = (a & EXPONENT_MASK_F_17_15) >> SIGNIFICAND_BITS_F_17_15;
    uint32_t exp_b = (b & EXPONENT_MASK_F_17_15) >> SIGNIFICAND_BITS_F_17_15;
    uint32_t sig_a = a & SIGNIFICAND_MASK_F_17_15;
    uint32_t sig_b = b & SIGNIFICAND_MASK_F_17_15;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_F_17_15 & sig_a)) {
        sig_a |= EXPONENT_MASK_F_17_15;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_F_17_15;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_F_17_15 & sig_b)) {
        sig_b |= EXPONENT_MASK_F_17_15;
    ...
    // Normalize result
    uint32_t result_sig = (uint32_t)temp_sig & SIGNIFICAND_MASK_F_17_15;
    if (msb > 0) {
        result_sig = temp_sig >> msb;
    } else if (msb < 0) {
        result_sig = temp_sig << abs(msb);
    }
    result_exp += msb - SIGNIFICAND_BITS_F_17_15 + 1;
    log_debug_float("result significand", result_sig);
...
f1715_t div_f1715(f1715_t a, f1715_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_F_17_15;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_F_17_15;
        }
    }

    // Perform division
    uint64_t temp_sig = ((uint64_t)sig_a << SIGNIFICAND_BITS_F_17_15) / sig_b;
...
int com_f1715(f1715_t a, f1715_t b) {
    ...
    int32_t exp_a = (a & EXPONENT_MASK_F_17_15) >> SIGNIFICAND_BITS_F_17_15;
    int32_t exp_b = (b & EXPONENT_MASK_F_17_15) >> SIGNIFICAND_BITS_F_17_15;
    int32_t sig_a = a & SIGNIFICAND_MASK_F_17_15;
    int32_t sig_b = b & SIGNIFICAND_MASK_F_17_15;
