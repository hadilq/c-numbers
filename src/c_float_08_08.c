...
static uint8_t get_msb_index_positive(uint16_t n) {
    ...
    uint16_t copy = n & SIGNIFICAND_MASK_F_08_08;
    for (uint8_t i = 1; i <= SIGNIFICAND_BITS_F_08_08; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_32(uint32_t n) {
    ...
    uint32_t copy = n & SIGNIFICAND_2_MASK_F_08_08;
    for (uint8_t i = 1; i <= 2 * SIGNIFICAND_BITS_F_08_08; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
f0808_t new_f0808(int16_t exp, int16_t sig) {
    return (exp << SIGNIFICAND_BITS_F_08_08) | (sig & SIGNIFICAND_MASK_F_08_08);
}
...
f0808_t new_f0808_from_float16(_Float16 f) {
    ...
    if (SIGNIFICAND_BITS_F_08_08 > Float16_SIG_1) {
        significand <<= SIGNIFICAND_BITS_F_08_08 - Float16_SIG_1;
    } else {
        significand >>= Float16_SIG_1 - SIGNIFICAND_BITS_F_08_08;
    }
    return new_f0808(exponent, significand);
}
  #endif
#endif
...
int16_t exp_f0808(f0808_t a) {
    uint16_t exp_a = (a & EXPONENT_MASK_F_08_08) >> SIGNIFICAND_BITS_F_08_08;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_F_08_08 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_F_08_08 << EXPONENT_BITS_F_08_08; // Fill the left with ones
    }
    return exp_a;
}
...
int16_t sig_f0808(f0808_t a) {
    uint16_t sig_a = a & SIGNIFICAND_MASK_F_08_08;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_F_08_08 & sig_a)) {
        sig_a |= EXPONENT_MASK_F_08_08;
    }
    return sig_a;
}
...
f0808_t add_f0808(f0808_t a, f0808_t b) {
    ...
    int16_t exp_a = (a & EXPONENT_MASK_F_08_08) >> SIGNIFICAND_BITS_F_08_08;
    int16_t exp_b = (b & EXPONENT_MASK_F_08_08) >> SIGNIFICAND_BITS_F_08_08;
    uint16_t sig_a = a & SIGNIFICAND_MASK_F_08_08;
    uint16_t sig_b = b & SIGNIFICAND_MASK_F_08_08;
    ...
    if ((SIGNIFICAND_SIGN_MASK_F_08_08 & sig_b)) {
        sig_b |= EXPONENT_MASK_F_08_08;
        posi_b = false;
    ...
    if ((EXPONENT_SIGN_MASK_F_08_08 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_F_08_08 << EXPONENT_BITS_F_08_08; // Fill the left with ones
    }
    ...
    if (exp_a > exp_b) {
        ...
        sig_b >>= diff;
        if (!posi_b) {
            sig_b |= ((1U << diff) - 1) << (BITS_F_08_08 - diff); // Fill the left with ones
            if ((sig_b & SIGNIFICAND_MASK_F_08_08) == SIGNIFICAND_MASK_F_08_08) {
                sig_b = 0;
            }
        }
        ...
        sig_a >>= diff;
        if (!posi_a) {
            sig_a |= ((1U << diff) - 1) << (BITS_F_08_08 - diff); // Fill the left with ones
            if ((sig_a & SIGNIFICAND_MASK_F_08_08) == SIGNIFICAND_MASK_F_08_08) {
                sig_a = 0;
            }
        }
    ...
    if (BITS_SIGN_MASK_F_08_08 & result_sig) {
       posi_result = false;
    }
    ...
    msb -= SIGNIFICAND_BITS_F_08_08 - 1;
    log_debug_float("msb", msb);
    ...
    // Assemble result
    return new_f0808(result_exp, result_sig);
}

f0808_t sub_f0808(f0808_t a, f0808_t b) {
    return add_f0808(a, neg_f0808(b));
}

f0808_t neg_f0808(f0808_t b) {
    // Negate b and add
    uint16_t exp_b = b & EXPONENT_MASK_F_08_08;
    uint16_t sig_b = b & SIGNIFICAND_MASK_F_08_08;

    // Flip all bits of significand for two's complement
    if ((SIGNIFICAND_SIGN_MASK_F_08_08 & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_F_08_08;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_F_08_08;
    }
...
f0808_t mul_f0808(f0808_t a, f0808_t b) {
    uint16_t exp_a = (a & EXPONENT_MASK_F_08_08) >> SIGNIFICAND_BITS_F_08_08;
    uint16_t exp_b = (b & EXPONENT_MASK_F_08_08) >> SIGNIFICAND_BITS_F_08_08;
    uint16_t sig_a = a & SIGNIFICAND_MASK_F_08_08;
    uint16_t sig_b = b & SIGNIFICAND_MASK_F_08_08;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_F_08_08 & sig_a)) {
        sig_a |= EXPONENT_MASK_F_08_08;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_F_08_08;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_F_08_08 & sig_b)) {
        sig_b |= EXPONENT_MASK_F_08_08;
    ...
    // Normalize result
    uint16_t result_sig = (uint16_t)temp_sig & SIGNIFICAND_MASK_F_08_08;
    if (msb > 0) {
        result_sig = temp_sig >> msb;
    } else if (msb < 0) {
        result_sig = temp_sig << abs(msb);
    }
    result_exp += msb - SIGNIFICAND_BITS_F_08_08 + 1;
    log_debug_float("result significand", result_sig);
...
f0808_t div_f0808(f0808_t a, f0808_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_F_08_08;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_F_08_08;
        }
    }

    // Perform division
    uint32_t temp_sig = ((uint32_t)sig_a << SIGNIFICAND_BITS_F_08_08) / sig_b;
...
int com_f0808(f0808_t a, f0808_t b) {
    ...
    int16_t exp_a = (a & EXPONENT_MASK_F_08_08) >> SIGNIFICAND_BITS_F_08_08;
    int16_t exp_b = (b & EXPONENT_MASK_F_08_08) >> SIGNIFICAND_BITS_F_08_08;
    int16_t sig_a = a & SIGNIFICAND_MASK_F_08_08;
    int16_t sig_b = b & SIGNIFICAND_MASK_F_08_08;
