...
static uint8_t get_msb_index(uint8_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_04_04 - 1; // Handle 0 as a special case

    uint8_t copy = n << EXPONENT_BITS_P_04_04;
    for (uint8_t i = SIGNIFICAND_BITS_P_04_04 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_16(uint16_t n) {
    ...
    uint16_t copy = n << (2 * EXPONENT_BITS_P_04_04);
    if (copy == 0U) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_04_04 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0U) {
            return i;
        }
...
p0404_t new_p0404(int8_t exp, int8_t sig) {
    return (exp << SIGNIFICAND_BITS_P_04_04) | (sig & SIGNIFICAND_MASK_P_04_04);
}
...
int8_t exp_p0404(p0404_t a) {
    ...
    if ((EXPONENT_SIGN_MASK_P_04_04 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_04_04 << EXPONENT_BITS_P_04_04; // Fill the left with ones
    }
    return exp_a;
}
...
int8_t sig_p0404(p0404_t a) {
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_04_04 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_04_04;
    }
    return sig_a;
}
...
p0404_t add_p0404(p0404_t a, p0404_t b) {
    ...
    if ((EXPONENT_SIGN_MASK_P_04_04 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_04_04 << EXPONENT_BITS_P_04_04; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p0404(result_exp, result_sig);
}

p0404_t sub_p0404(p0404_t a, p0404_t b) {
    return add_p0404(a, neg_p0404(b));
}

p0404_t neg_p0404(p0404_t b) {
    // Negate b and add
    uint8_t exp_b = b & EXPONENT_MASK_P_04_04;
    uint8_t sig_b = b & SIGNIFICAND_MASK_P_04_04;
...
p0404_t mul_p0404(p0404_t a, p0404_t b) {
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_04_04 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_04_04;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_04_04;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_04_04 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_04_04;
    ...
    // Normalize result
    uint8_t result_sig = (uint8_t)temp_sig & SIGNIFICAND_MASK_P_04_04;
    if (msb > 0) {
...
p0404_t div_p0404(p0404_t a, p0404_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_P_04_04;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_P_04_04;
        }
    }
    ...
    uint8_t mask_r = 1U;
    let((sig_b | 1U) != 0U);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_04_04; i++) {
        uint8_t mask_j = 2U;
        if (i > 0) {
            mask_r = 1U << (i - 1);
        }
...
int com_p0404(p0404_t a, p0404_t b) {
    ...
    int8_t exp_a = (a & EXPONENT_MASK_P_04_04) >> SIGNIFICAND_BITS_P_04_04;
    int8_t exp_b = (b & EXPONENT_MASK_P_04_04) >> SIGNIFICAND_BITS_P_04_04;
    int8_t sig_a = a & SIGNIFICAND_MASK_P_04_04;
    int8_t sig_b = b & SIGNIFICAND_MASK_P_04_04;
