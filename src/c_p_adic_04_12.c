#include "c_p_adic_04_12.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_04_12_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint16_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_04_12 - 1; // Handle 0 as a special case

    uint16_t copy = n << EXPONENT_BITS_P_04_12;
    for (uint8_t i = SIGNIFICAND_BITS_P_04_12 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_32(uint32_t n) {
    ...
    uint32_t copy = n << (2 * EXPONENT_BITS_P_04_12);
    if (copy == 0U) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_04_12 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0U) {
            return i;
        }
...
p0412_t new_p0412(int16_t exp, int16_t sig) {
    return (exp << SIGNIFICAND_BITS_P_04_12) | (sig & SIGNIFICAND_MASK_P_04_12);
}

// exponent part of p-adic 0412
int16_t exp_p0412(p0412_t a) {
    uint16_t exp_a = (a & EXPONENT_MASK_P_04_12) >> SIGNIFICAND_BITS_P_04_12;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_P_04_12 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_04_12 << EXPONENT_BITS_P_04_12; // Fill the left with ones
    }
    return exp_a;
}

// significand part of p-adic 0412
int16_t sig_p0412(p0412_t a) {
    uint16_t sig_a = a & SIGNIFICAND_MASK_P_04_12;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_P_04_12 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_04_12;
    }
    return sig_a;
}
...
p0412_t add_p0412(p0412_t a, p0412_t b) {
    ...
    int16_t exp_a = (a & EXPONENT_MASK_P_04_12) >> SIGNIFICAND_BITS_P_04_12;
    int16_t exp_b = (b & EXPONENT_MASK_P_04_12) >> SIGNIFICAND_BITS_P_04_12;
    uint16_t sig_a = a & SIGNIFICAND_MASK_P_04_12;
    uint16_t sig_b = b & SIGNIFICAND_MASK_P_04_12;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_04_12 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_04_12;
    ...
    if ((EXPONENT_SIGN_MASK_P_04_12 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_04_12 << EXPONENT_BITS_P_04_12; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p0412(result_exp, result_sig);
}

p0412_t sub_p0412(p0412_t a, p0412_t b) {
    return add_p0412(a, neg_p0412(b));
}

p0412_t neg_p0412(p0412_t b) {
    // Negate b and add
    uint16_t exp_b = b & EXPONENT_MASK_P_04_12;
    uint16_t sig_b = b & SIGNIFICAND_MASK_P_04_12;

    // Flip all bits of significand for two's complement
    if ((1ULL << (SIGNIFICAND_BITS_P_04_12 - 1) & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_04_12;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_04_12;
    }
...
p0412_t mul_p0412(p0412_t a, p0412_t b) {
    uint16_t exp_a = (a & EXPONENT_MASK_P_04_12) >> SIGNIFICAND_BITS_P_04_12;
    uint16_t exp_b = (b & EXPONENT_MASK_P_04_12) >> SIGNIFICAND_BITS_P_04_12;
    uint16_t sig_a = a & SIGNIFICAND_MASK_P_04_12;
    uint16_t sig_b = b & SIGNIFICAND_MASK_P_04_12;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_04_12 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_04_12;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_04_12;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_04_12 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_04_12;
    ...
    // Normalize result
    uint16_t result_sig = (uint16_t)temp_sig & SIGNIFICAND_MASK_P_04_12;
    if (msb > 0) {
...
p0412_t div_p0412(p0412_t a, p0412_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_P_04_12;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_P_04_12;
        }
    }
    ...
    uint16_t mask_r = 1U;
    let((sig_b | 1U) != 0U);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_04_12; i++) {
        uint16_t mask_j = 2U;
        if (i > 0) {
            mask_r = 1U << (i - 1);
        }
...
int com_p0412(p0412_t a, p0412_t b) {
    ...
    int16_t exp_a = (a & EXPONENT_MASK_P_04_12) >> SIGNIFICAND_BITS_P_04_12;
    int16_t exp_b = (b & EXPONENT_MASK_P_04_12) >> SIGNIFICAND_BITS_P_04_12;
    int16_t sig_a = a & SIGNIFICAND_MASK_P_04_12;
    int16_t sig_b = b & SIGNIFICAND_MASK_P_04_12;
