...
static uint8_t get_msb_index_positive(uint64_t n) {
    ...
    uint64_t copy = n & SIGNIFICAND_MASK_F_02_62;
    for (uint8_t i = 1; i <= SIGNIFICAND_BITS_F_02_62; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_128(__uint128_t n) {
    ...
    __uint128_t copy = n & SIGNIFICAND_2_MASK_F_02_62;
    for (uint8_t i = 1; i <= 2 * SIGNIFICAND_BITS_F_02_62; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
f0262_t new_f0262(int64_t exp, int64_t sig) {
    return (exp << SIGNIFICAND_BITS_F_02_62) | (sig & SIGNIFICAND_MASK_F_02_62);
}
...
f0262_t new_f0262_from_float64(_Float64 f) {
    ...
    if (SIGNIFICAND_BITS_F_02_62 > Float64_SIG_1) {
        significand <<= (SIGNIFICAND_BITS_F_02_62 - Float64_SIG_1);
    } else {
        significand >>= Float64_SIG_1 - SIGNIFICAND_BITS_F_02_62;
    }
    return new_f0262(exponent, significand);
...
f0262_t new_f0262_from_float32(_Float32 f) {
    ...
    if (SIGNIFICAND_BITS_F_02_62 > Float32_SIG_1) {
        significand <<= SIGNIFICAND_BITS_F_02_62 - Float32_SIG_1;
    } else {
        significand >>= Float32_SIG_1 - SIGNIFICAND_BITS_F_02_62;
    }
    return new_f0262(exponent, significand);
}
...
f0262_t new_f0262_from_float16(_Float16 f) {
    ...
    if (SIGNIFICAND_BITS_F_02_62 > Float16_SIG_1) {
        significand <<= SIGNIFICAND_BITS_F_02_62 - Float16_SIG_1;
    } else {
        significand >>= Float16_SIG_1 - SIGNIFICAND_BITS_F_02_62;
    }
    return new_f0262(exponent, significand);
}
...
int64_t exp_f0262(f0262_t a) {
    uint64_t exp_a = (a & EXPONENT_MASK_F_02_62) >> SIGNIFICAND_BITS_F_02_62;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_F_02_62 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_F_02_62 << EXPONENT_BITS_F_02_62; // Fill the left with ones
    }
    return exp_a;
}
...
int64_t sig_f0262(f0262_t a) {
    uint64_t sig_a = a & SIGNIFICAND_MASK_F_02_62;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_F_02_62 & sig_a)) {
        sig_a |= EXPONENT_MASK_F_02_62;
    }
    return sig_a;
}
...
f0262_t add_f0262(f0262_t a, f0262_t b) {
    ...
    int64_t exp_a = (a & EXPONENT_MASK_F_02_62) >> SIGNIFICAND_BITS_F_02_62;
    int64_t exp_b = (b & EXPONENT_MASK_F_02_62) >> SIGNIFICAND_BITS_F_02_62;
    uint64_t sig_a = a & SIGNIFICAND_MASK_F_02_62;
    uint64_t sig_b = b & SIGNIFICAND_MASK_F_02_62;
    ...
    if ((SIGNIFICAND_SIGN_MASK_F_02_62 & sig_b)) {
        sig_b |= EXPONENT_MASK_F_02_62;
        posi_b = false;
    ...
    if ((EXPONENT_SIGN_MASK_F_02_62 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_F_02_62 << EXPONENT_BITS_F_02_62; // Fill the left with ones
    }
    ...
    if (exp_a > exp_b) {
        ...
        sig_b >>= diff;
        if (!posi_b) {
            sig_b |= ((1ULL << diff) - 1) << (BITS_F_02_62 - diff); // Fill the left with ones
            if ((sig_b & SIGNIFICAND_MASK_F_02_62) == SIGNIFICAND_MASK_F_02_62) {
                sig_b = 0;
            }
        }
        ...
        sig_a >>= diff;
        if (!posi_a) {
            sig_a |= ((1ULL << diff) - 1) << (BITS_F_02_62 - diff); // Fill the left with ones
            if ((sig_a & SIGNIFICAND_MASK_F_02_62) == SIGNIFICAND_MASK_F_02_62) {
                sig_a = 0;
            }
        }
    ...
    if (BITS_SIGN_MASK_F_02_62 & result_sig) {
       posi_result = false;
    }
    ...
    msb -= SIGNIFICAND_BITS_F_02_62 - 1;
    ...
    // Assemble result
    return new_f0262(result_exp, result_sig);
}

f0262_t sub_f0262(f0262_t a, f0262_t b) {
    return add_f0262(a, neg_f0262(b));
}

f0262_t neg_f0262(f0262_t b) {
    // Negate b and add
    uint64_t exp_b = b & EXPONENT_MASK_F_02_62;
    uint64_t sig_b = b & SIGNIFICAND_MASK_F_02_62;

    // Flip all bits of significand for two's complement
    if ((SIGNIFICAND_SIGN_MASK_F_02_62 & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_F_02_62;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_F_02_62;
    }
...
f0262_t mul_f0262(f0262_t a, f0262_t b) {
    uint64_t exp_a = (a & EXPONENT_MASK_F_02_62) >> SIGNIFICAND_BITS_F_02_62;
    uint64_t exp_b = (b & EXPONENT_MASK_F_02_62) >> SIGNIFICAND_BITS_F_02_62;
    uint64_t sig_a = a & SIGNIFICAND_MASK_F_02_62;
    uint64_t sig_b = b & SIGNIFICAND_MASK_F_02_62;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_F_02_62 & sig_a)) {
        sig_a |= EXPONENT_MASK_F_02_62;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_F_02_62;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_F_02_62 & sig_b)) {
        sig_b |= EXPONENT_MASK_F_02_62;
    ...
    // Normalize result
    uint64_t result_sig = (uint64_t)temp_sig & SIGNIFICAND_MASK_F_02_62;
    if (msb > 0) {
        result_sig = temp_sig >> msb;
    } else if (msb < 0) {
        result_sig = temp_sig << abs(msb);
    }
    result_exp += msb - SIGNIFICAND_BITS_F_02_62 + 1;
    log_debug_float("result significand", result_sig);
...
f0262_t div_f0262(f0262_t a, f0262_t b) {
    ...
    if (sig_b == 0ULL) {
        if (posi_a) {
            ...
            return MAX_VALUE_F_02_62;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_F_02_62;
        }
    }

    // Perform division
    __uint128_t temp_sig = ((__uint128_t)sig_a << SIGNIFICAND_BITS_F_02_62) / sig_b;
...
int com_f0262(f0262_t a, f0262_t b) {
    ...
    int64_t exp_a = (a & EXPONENT_MASK_F_02_62) >> SIGNIFICAND_BITS_F_02_62;
    int64_t exp_b = (b & EXPONENT_MASK_F_02_62) >> SIGNIFICAND_BITS_F_02_62;
    int64_t sig_a = a & SIGNIFICAND_MASK_F_02_62;
    int64_t sig_b = b & SIGNIFICAND_MASK_F_02_62;
