#include "c_p_adic_02_62.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_02_62_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint64_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_02_62 - 1; // Handle 0 as a special case

    uint64_t copy = n << EXPONENT_BITS_P_02_62;
    for (uint8_t i = SIGNIFICAND_BITS_P_02_62 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_128(__uint128_t n) {
    ...
    __uint128_t copy = n << (2 * EXPONENT_BITS_P_02_62);
    if (copy == 0ULL) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_02_62 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0ULL) {
            return i;
        }
...
p0262_t new_p0262(int64_t exp, int64_t sig) {
    return (exp << SIGNIFICAND_BITS_P_02_62) | (sig & SIGNIFICAND_MASK_P_02_62);
}

// exponent part of p-adic 0262
int64_t exp_p0262(p0262_t a) {
    uint64_t exp_a = (a & EXPONENT_MASK_P_02_62) >> SIGNIFICAND_BITS_P_02_62;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_P_02_62 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_02_62 << EXPONENT_BITS_P_02_62; // Fill the left with ones
    }
    return exp_a;
}
...
int64_t sig_p0262(p0262_t a) {
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_02_62;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_P_02_62 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_02_62;
    }
    return sig_a;
}
...
p0262_t add_p0262(p0262_t a, p0262_t b) {
    ...
    int64_t exp_a = (a & EXPONENT_MASK_P_02_62) >> SIGNIFICAND_BITS_P_02_62;
    int64_t exp_b = (b & EXPONENT_MASK_P_02_62) >> SIGNIFICAND_BITS_P_02_62;
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_02_62;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_02_62;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_02_62 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_02_62;
    ...
    if ((EXPONENT_SIGN_MASK_P_02_62 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_02_62 << EXPONENT_BITS_P_02_62; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p0262(result_exp, result_sig);
}

p0262_t sub_p0262(p0262_t a, p0262_t b) {
    return add_p0262(a, neg_p0262(b));
}

p0262_t neg_p0262(p0262_t b) {
    // Negate b and add
    uint64_t exp_b = b & EXPONENT_MASK_P_02_62;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_02_62;

    // Flip all bits of significand for two's complement
    if ((SIGNIFICAND_SIGN_MASK_P_02_62 & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_02_62;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_02_62;
    }
...
p0262_t mul_p0262(p0262_t a, p0262_t b) {
    uint64_t exp_a = (a & EXPONENT_MASK_P_02_62) >> SIGNIFICAND_BITS_P_02_62;
    uint64_t exp_b = (b & EXPONENT_MASK_P_02_62) >> SIGNIFICAND_BITS_P_02_62;
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_02_62;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_02_62;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_02_62 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_02_62;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_02_62;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_02_62 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_02_62;
    ...
    // Normalize result
    uint64_t result_sig = (uint64_t)temp_sig & SIGNIFICAND_MASK_P_02_62;
    if (msb > 0) {
...
p0262_t div_p0262(p0262_t a, p0262_t b) {
    ...
    if (sig_b == 0ULL) {
        if (posi_a) {
            ...
            return MAX_VALUE_P_02_62;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_P_02_62;
        }
    }
    ...
    let((sig_b | 1ULL) != 0ULL);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_02_62; i++) {
...
int com_p0262(p0262_t a, p0262_t b) {
    ...
    int64_t exp_a = (a & EXPONENT_MASK_P_02_62) >> SIGNIFICAND_BITS_P_02_62;
    int64_t exp_b = (b & EXPONENT_MASK_P_02_62) >> SIGNIFICAND_BITS_P_02_62;
    int64_t sig_a = a & SIGNIFICAND_MASK_P_02_62;
    int64_t sig_b = b & SIGNIFICAND_MASK_P_02_62;
