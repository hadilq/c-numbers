...
static uint8_t get_msb_index_positive(uint8_t n) {
    ...
    uint8_t copy = n & SIGNIFICAND_MASK_F_03_05;
    for (uint8_t i = 1; i <= SIGNIFICAND_BITS_F_03_05; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_16(uint16_t n) {
    ...
    uint16_t copy = n & SIGNIFICAND_2_MASK_F_03_05;
    for (uint8_t i = 1; i <= 2 * SIGNIFICAND_BITS_F_03_05; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
f0305_t new_f0305(int8_t exp, int8_t sig) {
    return (exp << SIGNIFICAND_BITS_F_03_05) | (sig & SIGNIFICAND_MASK_F_03_05);
}

// exponent part of float 0305
int8_t exp_f0305(f0305_t a) {
    uint8_t exp_a = (a & EXPONENT_MASK_F_03_05) >> SIGNIFICAND_BITS_F_03_05;

    // Handle negatives exponents
    if (EXPONENT_SIGN_MASK_F_03_05 & exp_a) {
        exp_a |= SIGNIFICAND_MASK_F_03_05 << EXPONENT_BITS_F_03_05; // Fill the left with ones
    }
    return exp_a;
}
...
int8_t sig_f0305(f0305_t a) {
    uint8_t sig_a = a & SIGNIFICAND_MASK_F_03_05;

    // Handle negative numbers
    if (SIGNIFICAND_SIGN_MASK_F_03_05 & sig_a) {
        sig_a |= EXPONENT_MASK_F_03_05;
    }
    return sig_a;
}
...
f0305_t add_f0305(f0305_t a, f0305_t b) {
    ...
    int8_t exp_a = (a & EXPONENT_MASK_F_03_05) >> SIGNIFICAND_BITS_F_03_05;
    int8_t exp_b = (b & EXPONENT_MASK_F_03_05) >> SIGNIFICAND_BITS_F_03_05;
    uint8_t sig_a = a & SIGNIFICAND_MASK_F_03_05;
    uint8_t sig_b = b & SIGNIFICAND_MASK_F_03_05;
    ...
    if (SIGNIFICAND_SIGN_MASK_F_03_05 & sig_b) {
        sig_b |= EXPONENT_MASK_F_03_05;
        posi_b = false;
    ...
    if (EXPONENT_SIGN_MASK_F_03_05 & exp_b) {
        exp_b |= SIGNIFICAND_MASK_F_03_05 << EXPONENT_BITS_F_03_05; // Fill the left with ones
    }
    ...
    if (exp_a > exp_b) {
        ...
        sig_b >>= diff;
        if (!posi_b) {
            sig_b |= ((1U << diff) - 1) << (BITS_F_03_05 - diff); // Fill the left with ones
            if ((sig_b & SIGNIFICAND_MASK_F_03_05) == SIGNIFICAND_MASK_F_03_05) {
                sig_b = 0;
            }
        }
        ...
        sig_a >>= diff;
        if (!posi_a) {
            sig_a |= ((1U << diff) - 1) << (BITS_F_03_05 - diff); // Fill the left with ones
            if ((sig_a & SIGNIFICAND_MASK_F_03_05) == SIGNIFICAND_MASK_F_03_05) {
                sig_a = 0;
            }
        }
    ...
    if (BITS_SIGN_MASK_F_03_05 & result_sig) {
       posi_result = false;
    }
    ...
    msb -= SIGNIFICAND_BITS_F_03_05 - 1;
    ...
    // Assemble result
    return new_f0305(result_exp, result_sig);
}

f0305_t sub_f0305(f0305_t a, f0305_t b) {
    return add_f0305(a, neg_f0305(b));
}

f0305_t neg_f0305(f0305_t b) {
    // Negate b and add
    uint8_t exp_b = b & EXPONENT_MASK_F_03_05;
    uint8_t sig_b = b & SIGNIFICAND_MASK_F_03_05;

    // Flip all bits of significand for two's complement
    if ((1ULL << (SIGNIFICAND_BITS_F_03_05 - 1) & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_F_03_05;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_F_03_05;
    }
...
f0305_t mul_f0305(f0305_t a, f0305_t b) {
    uint8_t exp_a = (a & EXPONENT_MASK_F_03_05) >> SIGNIFICAND_BITS_F_03_05;
    uint8_t exp_b = (b & EXPONENT_MASK_F_03_05) >> SIGNIFICAND_BITS_F_03_05;
    uint8_t sig_a = a & SIGNIFICAND_MASK_F_03_05;
    uint8_t sig_b = b & SIGNIFICAND_MASK_F_03_05;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if (SIGNIFICAND_SIGN_MASK_F_03_05 & sig_a) {
        sig_a |= EXPONENT_MASK_F_03_05;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_F_03_05;
        log_debug_label("a is negative");
    }
    if (SIGNIFICAND_SIGN_MASK_F_03_05 & sig_b) {
        sig_b |= EXPONENT_MASK_F_03_05;
    ...
    // Normalize result
    uint8_t result_sig = (uint8_t)temp_sig & SIGNIFICAND_MASK_F_03_05;
    if (msb > 0) {
    ...
    result_exp += msb - SIGNIFICAND_BITS_F_03_05 + 1;
    log_debug_float("result significand", result_sig);
    log_debug_float("result exp", result_exp);
...
f0305_t div_f0305(f0305_t a, f0305_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_F_03_05;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_F_03_05;
        }
    }

    // Perform division
    uint16_t temp_sig = ((uint16_t)sig_a << SIGNIFICAND_BITS_F_03_05) / sig_b;
...
int com_f0305(f0305_t a, f0305_t b) {
    ...
    int8_t exp_a = (a & EXPONENT_MASK_F_03_05) >> SIGNIFICAND_BITS_F_03_05;
    int8_t exp_b = (b & EXPONENT_MASK_F_03_05) >> SIGNIFICAND_BITS_F_03_05;
    int8_t sig_a = a & SIGNIFICAND_MASK_F_03_05;
    int8_t sig_b = b & SIGNIFICAND_MASK_F_03_05;
