...
static uint8_t get_msb_index_positive(uint16_t n) {
    ...
    uint16_t copy = n & SIGNIFICAND_MASK_F_05_11;
    for (uint8_t i = 1; i <= SIGNIFICAND_BITS_F_05_11; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_32(uint32_t n) {
    ...
    uint32_t copy = n & SIGNIFICAND_2_MASK_F_05_11;
    for (uint8_t i = 1; i <= 2 * SIGNIFICAND_BITS_F_05_11; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
f0511_t new_f0511_from_float16(_Float16 f) {
    ...
    if (SIGNIFICAND_BITS_F_05_11 > Float16_SIG_1) {
        significand <<= SIGNIFICAND_BITS_F_05_11 - Float16_SIG_1;
    } else {
        significand >>= Float16_SIG_1 - SIGNIFICAND_BITS_F_05_11;
    }
...
int16_t exp_f0511(f0511_t a) {
    uint16_t exp_a = (a & EXPONENT_MASK_F_05_11) >> SIGNIFICAND_BITS_F_05_11;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_F_05_11 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_F_05_11 << EXPONENT_BITS_F_05_11; // Fill the left with ones
    }
    return exp_a;
}

// significand part of float 0511
int16_t sig_f0511(f0511_t a) {
    uint16_t sig_a = a & SIGNIFICAND_MASK_F_05_11;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_F_05_11 & sig_a)) {
        sig_a |= EXPONENT_MASK_F_05_11;
    }
    return sig_a;
}
...
f0511_t add_f0511(f0511_t a, f0511_t b) {
    ...
    int16_t exp_a = (a & EXPONENT_MASK_F_05_11) >> SIGNIFICAND_BITS_F_05_11;
    int16_t exp_b = (b & EXPONENT_MASK_F_05_11) >> SIGNIFICAND_BITS_F_05_11;
    uint16_t sig_a = a & SIGNIFICAND_MASK_F_05_11;
    uint16_t sig_b = b & SIGNIFICAND_MASK_F_05_11;
    ...
    if ((SIGNIFICAND_SIGN_MASK_F_05_11 & sig_b)) {
        sig_b |= EXPONENT_MASK_F_05_11;
        posi_b = false;
    ...
    if ((EXPONENT_SIGN_MASK_F_05_11 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_F_05_11 << EXPONENT_BITS_F_05_11; // Fill the left with ones
    }
    ...
    if (exp_a > exp_b) {
        ...
        sig_b >>= diff;
        if (!posi_b) {
            sig_b |= ((1U << diff) - 1) << (BITS_F_05_11 - diff); // Fill the left with ones
            if ((sig_b & SIGNIFICAND_MASK_F_05_11) == SIGNIFICAND_MASK_F_05_11) {
                sig_b = 0;
            }
        }
        ...
        sig_a >>= diff;
        if (!posi_a) {
            sig_a |= ((1U << diff) - 1) << (BITS_F_05_11 - diff); // Fill the left with ones
            if ((sig_a & SIGNIFICAND_MASK_F_05_11) == SIGNIFICAND_MASK_F_05_11) {
                sig_a = 0;
            }
        }
    ...
    if (BITS_SIGN_MASK_F_05_11 & result_sig) {
       posi_result = false;
    }
    ...
    msb -= SIGNIFICAND_BITS_F_05_11 - 1;
    log_debug_float("msb", msb);
    ...
    // Assemble result
    return new_f0511(result_exp, result_sig);
}

f0511_t sub_f0511(f0511_t a, f0511_t b) {
    return add_f0511(a, neg_f0511(b));
}

f0511_t neg_f0511(f0511_t b) {
    // Negate b and add
    uint16_t exp_b = b & EXPONENT_MASK_F_05_11;
    uint16_t sig_b = b & SIGNIFICAND_MASK_F_05_11;

    // Flip all bits of significand for two's complement
    if ((SIGNIFICAND_SIGN_MASK_F_05_11 & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_F_05_11;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_F_05_11;
    }
...
f0511_t mul_f0511(f0511_t a, f0511_t b) {
    uint16_t exp_a = (a & EXPONENT_MASK_F_05_11) >> SIGNIFICAND_BITS_F_05_11;
    uint16_t exp_b = (b & EXPONENT_MASK_F_05_11) >> SIGNIFICAND_BITS_F_05_11;
    uint16_t sig_a = a & SIGNIFICAND_MASK_F_05_11;
    uint16_t sig_b = b & SIGNIFICAND_MASK_F_05_11;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_F_05_11 & sig_a)) {
        sig_a |= EXPONENT_MASK_F_05_11;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_F_05_11;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_F_05_11 & sig_b)) {
        sig_b |= EXPONENT_MASK_F_05_11;
    ...
    // Normalize result
    uint16_t result_sig = (uint16_t)temp_sig & SIGNIFICAND_MASK_F_05_11;
    if (msb > 0) {
        result_sig = temp_sig >> msb;
    } else if (msb < 0) {
        result_sig = temp_sig << abs(msb);
    }
    result_exp += msb - SIGNIFICAND_BITS_F_05_11 + 1;
    log_debug_float("result significand", result_sig);
...
f0511_t div_f0511(f0511_t a, f0511_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_F_05_11;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_F_05_11;
        }
    }

    // Perform division
    uint32_t temp_sig = ((uint32_t)sig_a << SIGNIFICAND_BITS_F_05_11) / sig_b;
...
int com_f0511(f0511_t a, f0511_t b) {
    ...
    int16_t exp_a = (a & EXPONENT_MASK_F_05_11) >> SIGNIFICAND_BITS_F_05_11;
    int16_t exp_b = (b & EXPONENT_MASK_F_05_11) >> SIGNIFICAND_BITS_F_05_11;
    int16_t sig_a = a & SIGNIFICAND_MASK_F_05_11;
    int16_t sig_b = b & SIGNIFICAND_MASK_F_05_11;
