...
f1105_t new_f1105(int16_t exp, int16_t sig) {
    return (exp << SIGNIFICAND_BITS_F_11_05) | (sig & SIGNIFICAND_MASK_F_11_05);
}
...
f1105_t new_f1105_from_float16(_Float16 f) {
    float16_cast f1 = { .f = f };
    ...
    if (SIGNIFICAND_BITS_F_11_05 > Float16_SIG_1) {
        significand <<= SIGNIFICAND_BITS_F_11_05 - Float16_SIG_1;
    } else {
        significand >>= Float16_SIG_1 - SIGNIFICAND_BITS_F_11_05;
    }
    return new_f1105(exponent, significand);
}
  #endif
#endif

// exponent part of float 1105
int16_t exp_f1105(f1105_t a) {
    uint16_t exp_a = (a & EXPONENT_MASK_F_11_05) >> SIGNIFICAND_BITS_F_11_05;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_F_11_05 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_F_11_05 << EXPONENT_BITS_F_11_05; // Fill the left with ones
    }
    return exp_a;
}

// significand part of float 1105
int16_t sig_f1105(f1105_t a) {
    uint16_t sig_a = a & SIGNIFICAND_MASK_F_11_05;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_F_11_05 & sig_a)) {
        sig_a |= EXPONENT_MASK_F_11_05;
    }
    return sig_a;
}
...
f1105_t add_f1105(f1105_t a, f1105_t b) {
    ...
    if ((SIGNIFICAND_SIGN_MASK_F_11_05 & sig_b)) {
        sig_b |= EXPONENT_MASK_F_11_05;
        posi_b = false;
    ...
    if ((EXPONENT_SIGN_MASK_F_11_05 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_F_11_05 << EXPONENT_BITS_F_11_05; // Fill the left with ones
    }
    ...
    msb -= SIGNIFICAND_BITS_F_11_05 - 1;
    log_debug_float("msb", msb);
    ...
    // Assemble result
    return new_f1105(result_exp, result_sig);
}

f1105_t sub_f1105(f1105_t a, f1105_t b) {
    return add_f1105(a, neg_f1105(b));
}

f1105_t neg_f1105(f1105_t b) {
    // Negate b and add
    uint16_t exp_b = b & EXPONENT_MASK_F_11_05;
    uint16_t sig_b = b & SIGNIFICAND_MASK_F_11_05;

    // Flip all bits of significand for two's complement
    if ((SIGNIFICAND_SIGN_MASK_F_11_05 & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_F_11_05;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_F_11_05;
    }
...
f1105_t mul_f1105(f1105_t a, f1105_t b) {
    ...
    if ((SIGNIFICAND_SIGN_MASK_F_11_05 & sig_a)) {
        sig_a |= EXPONENT_MASK_F_11_05;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_F_11_05;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_F_11_05 & sig_b)) {
        sig_b |= EXPONENT_MASK_F_11_05;
    ...
    // Normalize result
    uint16_t result_sig = (uint16_t)temp_sig & SIGNIFICAND_MASK_F_11_05;
    if (msb > 0) {
        result_sig = temp_sig >> msb;
    } else if (msb < 0) {
        result_sig = temp_sig << abs(msb);
    }
    result_exp += msb - SIGNIFICAND_BITS_F_11_05 + 1;
    log_debug_float("result significand", result_sig);
...
f1105_t div_f1105(f1105_t a, f1105_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_F_11_05;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_F_11_05;
        }
    }

    // Perform division
    uint32_t temp_sig = ((uint32_t)sig_a << SIGNIFICAND_BITS_F_11_05) / sig_b;
...
int com_f1105(f1105_t a, f1105_t b) {
    ...
    int16_t exp_a = (a & EXPONENT_MASK_F_11_05) >> SIGNIFICAND_BITS_F_11_05;
    int16_t exp_b = (b & EXPONENT_MASK_F_11_05) >> SIGNIFICAND_BITS_F_11_05;
    int16_t sig_a = a & SIGNIFICAND_MASK_F_11_05;
    int16_t sig_b = b & SIGNIFICAND_MASK_F_11_05;
