...
static uint8_t get_msb_index_positive(uint32_t n) {
    ...
    uint32_t copy = n & SIGNIFICAND_MASK_F_22_10;
    for (uint8_t i = 1; i <= SIGNIFICAND_BITS_F_22_10; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_64(uint64_t n) {
    ...
    uint64_t copy = n & SIGNIFICAND_2_MASK_F_22_10;
    for (uint8_t i = 1; i <= 2 * SIGNIFICAND_BITS_F_22_10; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
f2210_t new_f2210(int32_t exp, int32_t sig) {
    return (exp << SIGNIFICAND_BITS_F_22_10) | (sig & SIGNIFICAND_MASK_F_22_10);
}
...
f2210_t new_f2210_from_float32(_Float32 f) {
    ...
    if (SIGNIFICAND_BITS_F_22_10 > Float32_SIG_1) {
        significand <<= SIGNIFICAND_BITS_F_22_10 - Float32_SIG_1;
    } else {
        significand >>= Float32_SIG_1 - SIGNIFICAND_BITS_F_22_10;
    }
    return new_f2210(exponent, significand);
}
...
f2210_t new_f2210_from_float16(_Float16 f) {
    ...
    if (SIGNIFICAND_BITS_F_22_10 > Float16_SIG_1) {
        significand <<= SIGNIFICAND_BITS_F_22_10 - Float16_SIG_1;
    } else {
        significand >>= Float16_SIG_1 - SIGNIFICAND_BITS_F_22_10;
    }
    return new_f2210(exponent, significand);
}
...
int32_t exp_f2210(f2210_t a) {
    uint32_t exp_a = (a & EXPONENT_MASK_F_22_10) >> SIGNIFICAND_BITS_F_22_10;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_F_22_10 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_F_22_10 << EXPONENT_BITS_F_22_10; // Fill the left with ones
    }
    return exp_a;
}
...
int32_t sig_f2210(f2210_t a) {
    uint32_t sig_a = a & SIGNIFICAND_MASK_F_22_10;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_F_22_10 & sig_a)) {
        sig_a |= EXPONENT_MASK_F_22_10;
    }
    return sig_a;
}
...
f2210_t add_f2210(f2210_t a, f2210_t b) {
    ...
    int32_t exp_a = (a & EXPONENT_MASK_F_22_10) >> SIGNIFICAND_BITS_F_22_10;
    int32_t exp_b = (b & EXPONENT_MASK_F_22_10) >> SIGNIFICAND_BITS_F_22_10;
    uint32_t sig_a = a & SIGNIFICAND_MASK_F_22_10;
    uint32_t sig_b = b & SIGNIFICAND_MASK_F_22_10;
    ...
    if ((SIGNIFICAND_SIGN_MASK_F_22_10 & sig_b)) {
        sig_b |= EXPONENT_MASK_F_22_10;
        posi_b = false;
    ...
    if ((EXPONENT_SIGN_MASK_F_22_10 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_F_22_10 << EXPONENT_BITS_F_22_10; // Fill the left with ones
    }
    ...
    if (exp_a > exp_b) {
        ...
        sig_b >>= diff;
        if (!posi_b) {
            sig_b |= ((1U << diff) - 1) << (BITS_F_22_10 - diff); // Fill the left with ones
            if ((sig_b & SIGNIFICAND_MASK_F_22_10) == SIGNIFICAND_MASK_F_22_10) {
                sig_b = 0;
            }
        }
        ...
        sig_a >>= diff;
        if (!posi_a) {
            sig_a |= ((1U << diff) - 1) << (BITS_F_22_10 - diff); // Fill the left with ones
            if ((sig_a & SIGNIFICAND_MASK_F_22_10) == SIGNIFICAND_MASK_F_22_10) {
                sig_a = 0;
            }
        }
    ...
    if (BITS_SIGN_MASK_F_22_10 & result_sig) {
       posi_result = false;
    }
    ...
    msb -= SIGNIFICAND_BITS_F_22_10 - 1;
    ...
    // Assemble result
    return new_f2210(result_exp, result_sig);
}

f2210_t sub_f2210(f2210_t a, f2210_t b) {
    return add_f2210(a, neg_f2210(b));
}

f2210_t neg_f2210(f2210_t b) {
    // Negate b and add
    uint32_t exp_b = b & EXPONENT_MASK_F_22_10;
    uint32_t sig_b = b & SIGNIFICAND_MASK_F_22_10;

    // Flip all bits of significand for two's complement
    if ((SIGNIFICAND_SIGN_MASK_F_22_10 & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_F_22_10;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_F_22_10;
    }
...
f2210_t mul_f2210(f2210_t a, f2210_t b) {
    uint32_t exp_a = (a & EXPONENT_MASK_F_22_10) >> SIGNIFICAND_BITS_F_22_10;
    uint32_t exp_b = (b & EXPONENT_MASK_F_22_10) >> SIGNIFICAND_BITS_F_22_10;
    uint32_t sig_a = a & SIGNIFICAND_MASK_F_22_10;
    uint32_t sig_b = b & SIGNIFICAND_MASK_F_22_10;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_F_22_10 & sig_a)) {
        sig_a |= EXPONENT_MASK_F_22_10;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_F_22_10;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_F_22_10 & sig_b)) {
        sig_b |= EXPONENT_MASK_F_22_10;
    ...
    // Normalize result
    uint32_t result_sig = (uint32_t)temp_sig & SIGNIFICAND_MASK_F_22_10;
    if (msb > 0) {
        result_sig = temp_sig >> msb;
    } else if (msb < 0) {
        result_sig = temp_sig << abs(msb);
    }
    result_exp += msb - SIGNIFICAND_BITS_F_22_10 + 1;
    log_debug_float("result significand", result_sig);
...
f2210_t div_f2210(f2210_t a, f2210_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_F_22_10;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_F_22_10;
        }
    }

    // Perform division
    uint64_t temp_sig = ((uint64_t)sig_a << SIGNIFICAND_BITS_F_22_10) / sig_b;
...
int com_f2210(f2210_t a, f2210_t b) {
    ...
    int32_t exp_a = (a & EXPONENT_MASK_F_22_10) >> SIGNIFICAND_BITS_F_22_10;
    int32_t exp_b = (b & EXPONENT_MASK_F_22_10) >> SIGNIFICAND_BITS_F_22_10;
    int32_t sig_a = a & SIGNIFICAND_MASK_F_22_10;
    int32_t sig_b = b & SIGNIFICAND_MASK_F_22_10;
