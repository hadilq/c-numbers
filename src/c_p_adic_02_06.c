...
p0206_t div_p0206(p0206_t a, p0206_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return (((1U << (EXPONENT_BITS_P_02_06 - 2)) - 1) << SIGNIFICAND_BITS_P_02_06)
                   | ((1U << (SIGNIFICAND_BITS_P_02_06 - 2)) - 1);
        } else {
            // Return min value as "-infinity"
            return (((1U << (EXPONENT_BITS_P_02_06 - 2)) - 1) << SIGNIFICAND_BITS_P_02_06)
                   | (SIGNIFICAND_SIGN_MASK_P_02_06);
        }
    }
