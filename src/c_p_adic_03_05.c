#include "c_p_adic_03_05.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_03_05_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint8_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_03_05 - 1; // Handle 0 as a special case

    uint8_t copy = n << EXPONENT_BITS_P_03_05;
    for (uint8_t i = SIGNIFICAND_BITS_P_03_05 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_16(uint16_t n) {
    ...
    uint16_t copy = n << (2 * EXPONENT_BITS_P_03_05);
    if (copy == 0U) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_03_05 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0U) {
            return i;
        }
...
p0305_t new_p0305(int8_t exp, int8_t sig) {
    return (exp << SIGNIFICAND_BITS_P_03_05) | (sig & SIGNIFICAND_MASK_P_03_05);
}

// exponent part of p-adic 0305
int8_t exp_p0305(p0305_t a) {
    uint8_t exp_a = (a & EXPONENT_MASK_P_03_05) >> SIGNIFICAND_BITS_P_03_05;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_P_03_05 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_03_05 << EXPONENT_BITS_P_03_05; // Fill the left with ones
    }
    return exp_a;
}
...
int8_t sig_p0305(p0305_t a) {
    uint8_t sig_a = a & SIGNIFICAND_MASK_P_03_05;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_P_03_05 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_03_05;
    }
    return sig_a;
}
...
p0305_t add_p0305(p0305_t a, p0305_t b) {
    ...
    int8_t exp_a = (a & EXPONENT_MASK_P_03_05) >> SIGNIFICAND_BITS_P_03_05;
    int8_t exp_b = (b & EXPONENT_MASK_P_03_05) >> SIGNIFICAND_BITS_P_03_05;
    uint8_t sig_a = a & SIGNIFICAND_MASK_P_03_05;
    uint8_t sig_b = b & SIGNIFICAND_MASK_P_03_05;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_03_05 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_03_05;
    ...
    if ((EXPONENT_SIGN_MASK_P_03_05 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_03_05 << EXPONENT_BITS_P_03_05; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p0305(result_exp, result_sig);
}

p0305_t sub_p0305(p0305_t a, p0305_t b) {
    return add_p0305(a, neg_p0305(b));
}

p0305_t neg_p0305(p0305_t b) {
    // Negate b and add
    uint8_t exp_b = b & EXPONENT_MASK_P_03_05;
    uint8_t sig_b = b & SIGNIFICAND_MASK_P_03_05;

    // Flip all bits of significand for two's complement
    if (SIGNIFICAND_SIGN_MASK_P_03_05 & sig_b) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_03_05;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_03_05;
    }
...
p0305_t mul_p0305(p0305_t a, p0305_t b) {
    uint8_t exp_a = (a & EXPONENT_MASK_P_03_05) >> SIGNIFICAND_BITS_P_03_05;
    uint8_t exp_b = (b & EXPONENT_MASK_P_03_05) >> SIGNIFICAND_BITS_P_03_05;
    uint8_t sig_a = a & SIGNIFICAND_MASK_P_03_05;
    uint8_t sig_b = b & SIGNIFICAND_MASK_P_03_05;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_03_05 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_03_05;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_03_05;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_03_05 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_03_05;
    ...
    // Normalize result
    uint8_t result_sig = (uint8_t)temp_sig & SIGNIFICAND_MASK_P_03_05;
    if (msb > 0) {
...
p0305_t div_p0305(p0305_t a, p0305_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_P_03_05;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_P_03_05;
        }
    }
    ...
    uint8_t mask_r = 1U;
    let((sig_b | 1U) != 0U);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_03_05; i++) {
        uint8_t mask_j = 2U;
        if (i > 0) {
            mask_r = 1U << (i - 1);
        }
...
int com_p0305(p0305_t a, p0305_t b) {
    ...
    int8_t exp_a = (a & EXPONENT_MASK_P_03_05) >> SIGNIFICAND_BITS_P_03_05;
    int8_t exp_b = (b & EXPONENT_MASK_P_03_05) >> SIGNIFICAND_BITS_P_03_05;
    int8_t sig_a = a & SIGNIFICAND_MASK_P_03_05;
    int8_t sig_b = b & SIGNIFICAND_MASK_P_03_05;
