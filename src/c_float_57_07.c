...
static uint8_t get_msb_index_positive(uint64_t n) {
    ...
    uint64_t copy = n & SIGNIFICAND_MASK_F_57_07;
...
static uint8_t get_msb_index_128(__uint128_t n) {
    ...
    __uint128_t copy = n & SIGNIFICAND_2_MASK_F_57_07;
    for (uint8_t i = 1; i <= 2 * SIGNIFICAND_BITS_F_57_07; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
f5707_t new_f5707(int64_t exp, int64_t sig) {
    return (exp << SIGNIFICAND_BITS_F_57_07) | (sig & SIGNIFICAND_MASK_F_57_07);
}
...
// construct float 5707
f5707_t new_f5707_from_float64(_Float64 f) {
    float64_cast f1 = { .f = f };
    ...
    if (SIGNIFICAND_BITS_F_57_07 > Float64_SIG_1) {
        significand <<= (SIGNIFICAND_BITS_F_57_07 - Float64_SIG_1);
    } else {
        significand >>= Float64_SIG_1 - SIGNIFICAND_BITS_F_57_07;
    }
    return new_f5707(exponent, significand);
...
// construct float 5707
f5707_t new_f5707_from_float32(_Float32 f) {
    float32_cast f1 = { .f = f };
    uint64_t significand = f1.parts.significand;
    uint64_t sign = f1.parts.sign;
    ...
    if (SIGNIFICAND_BITS_F_57_07 > Float32_SIG_1) {
        significand <<= SIGNIFICAND_BITS_F_57_07 - Float32_SIG_1;
    } else {
        significand >>= Float32_SIG_1 - SIGNIFICAND_BITS_F_57_07;
    }
    return new_f5707(exponent, significand);
}
...
// construct float 5707
f5707_t new_f5707_from_float16(_Float16 f) {
    float16_cast f1 = { .f = f };
    uint64_t significand = f1.parts.significand;
    ...
    if (SIGNIFICAND_BITS_F_57_07 > Float16_SIG_1) {
        significand <<= SIGNIFICAND_BITS_F_57_07 - Float16_SIG_1;
    } else {
        significand >>= Float16_SIG_1 - SIGNIFICAND_BITS_F_57_07;
    }
    return new_f5707(exponent, significand);
}

// exponent part of float 5707
int64_t exp_f5707(f5707_t a) {
...
f5707_t add_f5707(f5707_t a, f5707_t b) {
    ...
    int64_t exp_a = (a & EXPONENT_MASK_F_57_07) >> SIGNIFICAND_BITS_F_57_07;
    int64_t exp_b = (b & EXPONENT_MASK_F_57_07) >> SIGNIFICAND_BITS_F_57_07;
    uint64_t sig_a = a & SIGNIFICAND_MASK_F_57_07;
    ...
    if (exp_a > exp_b) {
        ...
        sig_b >>= diff;
        if (!posi_b) {
            sig_b |= ((1ULL << diff) - 1) << (BITS_F_57_07 - diff); // Fill the left with ones
            if ((sig_b & SIGNIFICAND_MASK_F_57_07) == SIGNIFICAND_MASK_F_57_07) {
                sig_b = 0;
            }
        }
        ...
        sig_a >>= diff;
        if (!posi_a) {
            sig_a |= ((1ULL << diff) - 1) << (BITS_F_57_07 - diff); // Fill the left with ones
            if ((sig_a & SIGNIFICAND_MASK_F_57_07) == SIGNIFICAND_MASK_F_57_07) {
                sig_a = 0;
            }
        }
    ...
    if (BITS_SIGN_MASK_F_57_07 & result_sig) {
       posi_result = false;
    }
    ...
    msb -= SIGNIFICAND_BITS_F_57_07 - 1;
    ...
    // Assemble result
    return new_f5707(result_exp, result_sig);
}

f5707_t sub_f5707(f5707_t a, f5707_t b) {
    return add_f5707(a, neg_f5707(b));
}

f5707_t neg_f5707(f5707_t b) {
    // Negate b and add
    uint64_t exp_b = b & EXPONENT_MASK_F_57_07;
    uint64_t sig_b = b & SIGNIFICAND_MASK_F_57_07;
...
f5707_t mul_f5707(f5707_t a, f5707_t b) {
    uint64_t exp_a = (a & EXPONENT_MASK_F_57_07) >> SIGNIFICAND_BITS_F_57_07;
    uint64_t exp_b = (b & EXPONENT_MASK_F_57_07) >> SIGNIFICAND_BITS_F_57_07;
    uint64_t sig_a = a & SIGNIFICAND_MASK_F_57_07;
    uint64_t sig_b = b & SIGNIFICAND_MASK_F_57_07;
    bool posi_a = true;
    bool posi_b = true;
    ...
    // Normalize result
    uint64_t result_sig = (uint64_t)temp_sig & SIGNIFICAND_MASK_F_57_07;
    if (msb > 0) {
        result_sig = temp_sig >> msb;
    } else if (msb < 0) {
        result_sig = temp_sig << abs(msb);
    }
    result_exp += msb - SIGNIFICAND_BITS_F_57_07 + 1;
    log_debug_float("result significand", result_sig);
...
f5707_t div_f5707(f5707_t a, f5707_t b) {
    ...
    if (sig_b == 0ULL) {
        if (posi_a) {
            ...
            return MAX_VALUE_F_57_07;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_F_57_07;
        }
    }

    // Perform division
    __uint128_t temp_sig = ((__uint128_t)sig_a << SIGNIFICAND_BITS_F_57_07) / sig_b;
...
int com_f5707(f5707_t a, f5707_t b) {
    ...
    int64_t exp_a = (a & EXPONENT_MASK_F_57_07) >> SIGNIFICAND_BITS_F_57_07;
    int64_t exp_b = (b & EXPONENT_MASK_F_57_07) >> SIGNIFICAND_BITS_F_57_07;
    int64_t sig_a = a & SIGNIFICAND_MASK_F_57_07;
    int64_t sig_b = b & SIGNIFICAND_MASK_F_57_07;
