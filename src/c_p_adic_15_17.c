#include "c_p_adic_15_17.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_15_17_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint32_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_15_17 - 1; // Handle 0 as a special case

    uint32_t copy = n << EXPONENT_BITS_P_15_17;
    for (uint8_t i = SIGNIFICAND_BITS_P_15_17 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_64(uint64_t n) {
    ...
    uint64_t copy = n << (2 * EXPONENT_BITS_P_15_17);
    if (copy == 0U) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_15_17 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0U) {
            return i;
        }
...
p1517_t new_p1517(uint32_t exp, uint32_t sig) {
    return (exp << SIGNIFICAND_BITS_P_15_17) | (sig & SIGNIFICAND_MASK_P_15_17);
}

// exponent part of p-adic 1517
uint32_t exp_p1517(p1517_t a) {
    uint32_t exp_a = (a & EXPONENT_MASK_P_15_17) >> SIGNIFICAND_BITS_P_15_17;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_P_15_17 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_15_17 << EXPONENT_BITS_P_15_17; // Fill the left with ones
    }
    return exp_a;
}

// significand part of p-adic 1517
uint32_t sig_p1517(p1517_t a) {
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_15_17;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_P_15_17 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_15_17;
    }
    return sig_a;
}
...
p1517_t add_p1517(p1517_t a, p1517_t b) {
    ...
    int32_t exp_a = (a & EXPONENT_MASK_P_15_17) >> SIGNIFICAND_BITS_P_15_17;
    int32_t exp_b = (b & EXPONENT_MASK_P_15_17) >> SIGNIFICAND_BITS_P_15_17;
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_15_17;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_15_17;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_15_17 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_15_17;
    ...
    if ((EXPONENT_SIGN_MASK_P_15_17 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_15_17 << EXPONENT_BITS_P_15_17; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p1517(result_exp, result_sig);
}

p1517_t sub_p1517(p1517_t a, p1517_t b) {
    return add_p1517(a, neg_p1517(b));
}

p1517_t neg_p1517(p1517_t b) {
    // Negate b and add
    uint32_t exp_b = b & EXPONENT_MASK_P_15_17;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_15_17;

    // Flip all bits of significand for two's complement
    if ((1ULL << (SIGNIFICAND_BITS_P_15_17 - 1) & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_15_17;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_15_17;
    }
...
p1517_t mul_p1517(p1517_t a, p1517_t b) {
    uint32_t exp_a = (a & EXPONENT_MASK_P_15_17) >> SIGNIFICAND_BITS_P_15_17;
    uint32_t exp_b = (b & EXPONENT_MASK_P_15_17) >> SIGNIFICAND_BITS_P_15_17;
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_15_17;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_15_17;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_15_17 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_15_17;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_15_17;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_15_17 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_15_17;
    ...
    // Normalize result
    uint32_t result_sig = (uint32_t)temp_sig & SIGNIFICAND_MASK_P_15_17;
    if (msb > 0) {
...
p1517_t div_p1517(p1517_t a, p1517_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return (((1U << (EXPONENT_BITS_P_15_17 - 2)) - 1) << SIGNIFICAND_BITS_P_15_17)
                   | ((1U << (SIGNIFICAND_BITS_P_15_17 - 2)) - 1);
        } else {
            // Return min value as "-infinity"
            return (((1U << (EXPONENT_BITS_P_15_17 - 2)) - 1) << SIGNIFICAND_BITS_P_15_17)
                   | (SIGNIFICAND_SIGN_MASK_P_15_17);
        }
    }
    ...
    uint32_t mask_r = 1U;
    let((sig_b | 1U) != 0U);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_15_17; i++) {
        uint32_t mask_j = 2U;
        if (i > 0) {
            mask_r = 1U << (i - 1);
        }
