#include "c_p_adic_56_08.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_56_08_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint64_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_56_08 - 1; // Handle 0 as a special case

    uint64_t copy = n << EXPONENT_BITS_P_56_08;
    for (uint8_t i = SIGNIFICAND_BITS_P_56_08 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_128(__uint128_t n) {
    ...
    __uint128_t copy = n << (2 * EXPONENT_BITS_P_56_08);
    if (copy == 0ULL) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_56_08 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0ULL) {
            return i;
        }
...
p5608_t new_p5608(int64_t exp, int64_t sig) {
    return (exp << SIGNIFICAND_BITS_P_56_08) | (sig & SIGNIFICAND_MASK_P_56_08);
}

// exponent part of p-adic 5608
int64_t exp_p5608(p5608_t a) {
    uint64_t exp_a = (a & EXPONENT_MASK_P_56_08) >> SIGNIFICAND_BITS_P_56_08;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_P_56_08 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_56_08 << EXPONENT_BITS_P_56_08; // Fill the left with ones
    }
    return exp_a;
}

// significand part of p-adic 5608
int64_t sig_p5608(p5608_t a) {
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_56_08;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_P_56_08 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_56_08;
    }
    return sig_a;
}
...
p5608_t add_p5608(p5608_t a, p5608_t b) {
    ...
    int64_t exp_a = (a & EXPONENT_MASK_P_56_08) >> SIGNIFICAND_BITS_P_56_08;
    int64_t exp_b = (b & EXPONENT_MASK_P_56_08) >> SIGNIFICAND_BITS_P_56_08;
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_56_08;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_56_08;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_56_08 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_56_08;
    ...
    if ((EXPONENT_SIGN_MASK_P_56_08 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_56_08 << EXPONENT_BITS_P_56_08; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p5608(result_exp, result_sig);
}

p5608_t sub_p5608(p5608_t a, p5608_t b) {
    return add_p5608(a, neg_p5608(b));
}

p5608_t neg_p5608(p5608_t b) {
    // Negate b and add
    uint64_t exp_b = b & EXPONENT_MASK_P_56_08;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_56_08;

    // Flip all bits of significand for two's complement
    if ((SIGNIFICAND_SIGN_MASK_P_56_08 & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_56_08;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_56_08;
    }
...
p5608_t mul_p5608(p5608_t a, p5608_t b) {
    uint64_t exp_a = (a & EXPONENT_MASK_P_56_08) >> SIGNIFICAND_BITS_P_56_08;
    uint64_t exp_b = (b & EXPONENT_MASK_P_56_08) >> SIGNIFICAND_BITS_P_56_08;
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_56_08;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_56_08;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_56_08 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_56_08;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_56_08;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_56_08 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_56_08;
    ...
    // Normalize result
    uint64_t result_sig = (uint64_t)temp_sig & SIGNIFICAND_MASK_P_56_08;
    if (msb > 0) {
...
p5608_t div_p5608(p5608_t a, p5608_t b) {
    ...
    if (sig_b == 0ULL) {
        if (posi_a) {
            ...
            return MAX_VALUE_P_56_08;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_P_56_08;
        }
    }
    ...
    let((sig_b | 1ULL) != 0ULL);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_56_08; i++) {
...
int com_p5608(p5608_t a, p5608_t b) {
    ...
    int64_t exp_a = (a & EXPONENT_MASK_P_56_08) >> SIGNIFICAND_BITS_P_56_08;
    int64_t exp_b = (b & EXPONENT_MASK_P_56_08) >> SIGNIFICAND_BITS_P_56_08;
    int64_t sig_a = a & SIGNIFICAND_MASK_P_56_08;
    int64_t sig_b = b & SIGNIFICAND_MASK_P_56_08;
