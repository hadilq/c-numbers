#include "c_p_adic_53_11.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_53_11_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint64_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_53_11 - 1; // Handle 0 as a special case

    uint64_t copy = n << EXPONENT_BITS_P_53_11;
    for (uint8_t i = SIGNIFICAND_BITS_P_53_11 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_128(__uint128_t n) {
    ...
    __uint128_t copy = n << (2 * EXPONENT_BITS_P_53_11);
    if (copy == 0ULL) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_53_11 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0ULL) {
            return i;
        }
...
p5311_t new_p5311(uint64_t exp, uint64_t sig) {
    return (exp << SIGNIFICAND_BITS_P_53_11) | (sig & SIGNIFICAND_MASK_P_53_11);
}

// exponent part of p-adic 5311
uint64_t exp_p5311(p5311_t a) {
    uint64_t exp_a = (a & EXPONENT_MASK_P_53_11) >> SIGNIFICAND_BITS_P_53_11;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_P_53_11 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_53_11 << EXPONENT_BITS_P_53_11; // Fill the left with ones
    }
    return exp_a;
}

// significand part of p-adic 5311
uint64_t sig_p5311(p5311_t a) {
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_53_11;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_P_53_11 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_53_11;
    }
    return sig_a;
}
...
p5311_t add_p5311(p5311_t a, p5311_t b) {
    ...
    int64_t exp_a = (a & EXPONENT_MASK_P_53_11) >> SIGNIFICAND_BITS_P_53_11;
    int64_t exp_b = (b & EXPONENT_MASK_P_53_11) >> SIGNIFICAND_BITS_P_53_11;
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_53_11;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_53_11;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_53_11 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_53_11;
    ...
    if ((EXPONENT_SIGN_MASK_P_53_11 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_53_11 << EXPONENT_BITS_P_53_11; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p5311(result_exp, result_sig);
}

p5311_t sub_p5311(p5311_t a, p5311_t b) {
    return add_p5311(a, neg_p5311(b));
}

p5311_t neg_p5311(p5311_t b) {
    // Negate b and add
    uint64_t exp_b = b & EXPONENT_MASK_P_53_11;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_53_11;

    // Flip all bits of significand for two's complement
    if ((SIGNIFICAND_SIGN_MASK_P_53_11 & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_53_11;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_53_11;
    }
...
p5311_t mul_p5311(p5311_t a, p5311_t b) {
    uint64_t exp_a = (a & EXPONENT_MASK_P_53_11) >> SIGNIFICAND_BITS_P_53_11;
    uint64_t exp_b = (b & EXPONENT_MASK_P_53_11) >> SIGNIFICAND_BITS_P_53_11;
    uint64_t sig_a = a & SIGNIFICAND_MASK_P_53_11;
    uint64_t sig_b = b & SIGNIFICAND_MASK_P_53_11;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_53_11 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_53_11;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_53_11;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_53_11 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_53_11;
    ...
    // Normalize result
    uint64_t result_sig = (uint64_t)temp_sig & SIGNIFICAND_MASK_P_53_11;
    if (msb > 0) {
...
p5311_t div_p5311(p5311_t a, p5311_t b) {
    ...
    if (sig_b == 0ULL) {
        if (posi_a) {
            ...
            return (((1ULL << (EXPONENT_BITS_P_53_11 - 2)) - 1) << SIGNIFICAND_BITS_P_53_11)
                   | ((1ULL << (SIGNIFICAND_BITS_P_53_11 - 2)) - 1);
        } else {
            // Return min value as "-infinity"
            return (((1ULL << (EXPONENT_BITS_P_53_11 - 2)) - 1) << SIGNIFICAND_BITS_P_53_11)
                   | (SIGNIFICAND_SIGN_MASK_P_53_11);
        }
    }
    ...
    let((sig_b | 1ULL) != 0ULL);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_53_11; i++) {
