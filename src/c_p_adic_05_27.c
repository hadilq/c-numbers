#include "c_p_adic_05_27.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_05_27_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint32_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_05_27 - 1; // Handle 0 as a special case

    uint32_t copy = n << EXPONENT_BITS_P_05_27;
    for (uint8_t i = SIGNIFICAND_BITS_P_05_27 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_64(uint64_t n) {
    ...
    uint64_t copy = n << (2 * EXPONENT_BITS_P_05_27);
    if (copy == 0U) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_05_27 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0U) {
            return i;
        }
...
p0527_t new_p0527(uint32_t exp, uint32_t sig) {
    return (exp << SIGNIFICAND_BITS_P_05_27) | (sig & SIGNIFICAND_MASK_P_05_27);
}

// exponent part of p-adic 0527
uint32_t exp_p0527(p0527_t a) {
    uint32_t exp_a = (a & EXPONENT_MASK_P_05_27) >> SIGNIFICAND_BITS_P_05_27;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_P_05_27 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_05_27 << EXPONENT_BITS_P_05_27; // Fill the left with ones
    }
    return exp_a;
}

// significand part of p-adic 0527
uint32_t sig_p0527(p0527_t a) {
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_05_27;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_P_05_27 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_05_27;
    }
    return sig_a;
}
...
p0527_t add_p0527(p0527_t a, p0527_t b) {
    ...
    int32_t exp_a = (a & EXPONENT_MASK_P_05_27) >> SIGNIFICAND_BITS_P_05_27;
    int32_t exp_b = (b & EXPONENT_MASK_P_05_27) >> SIGNIFICAND_BITS_P_05_27;
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_05_27;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_05_27;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_05_27 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_05_27;
    ...
    if ((EXPONENT_SIGN_MASK_P_05_27 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_05_27 << EXPONENT_BITS_P_05_27; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p0527(result_exp, result_sig);
}

p0527_t sub_p0527(p0527_t a, p0527_t b) {
    return add_p0527(a, neg_p0527(b));
}

p0527_t neg_p0527(p0527_t b) {
    // Negate b and add
    uint32_t exp_b = b & EXPONENT_MASK_P_05_27;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_05_27;

    // Flip all bits of significand for two's complement
    if ((1ULL << (SIGNIFICAND_BITS_P_05_27 - 1) & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_05_27;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_05_27;
    }
...
p0527_t mul_p0527(p0527_t a, p0527_t b) {
    uint32_t exp_a = (a & EXPONENT_MASK_P_05_27) >> SIGNIFICAND_BITS_P_05_27;
    uint32_t exp_b = (b & EXPONENT_MASK_P_05_27) >> SIGNIFICAND_BITS_P_05_27;
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_05_27;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_05_27;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_05_27 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_05_27;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_05_27;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_05_27 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_05_27;
    ...
    // Normalize result
    uint32_t result_sig = (uint32_t)temp_sig & SIGNIFICAND_MASK_P_05_27;
    if (msb > 0) {
...
p0527_t div_p0527(p0527_t a, p0527_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return (((1U << (EXPONENT_BITS_P_05_27 - 2)) - 1) << SIGNIFICAND_BITS_P_05_27)
                   | ((1U << (SIGNIFICAND_BITS_P_05_27 - 2)) - 1);
        } else {
            // Return min value as "-infinity"
            return (((1U << (EXPONENT_BITS_P_05_27 - 2)) - 1) << SIGNIFICAND_BITS_P_05_27)
                   | (SIGNIFICAND_SIGN_MASK_P_05_27);
        }
    }
    ...
    uint32_t mask_r = 1U;
    let((sig_b | 1U) != 0U);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_05_27; i++) {
        uint32_t mask_j = 2U;
        if (i > 0) {
            mask_r = 1U << (i - 1);
        }
