#include "c_p_adic_25_07.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_25_07_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint32_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_25_07 - 1; // Handle 0 as a special case

    uint32_t copy = n << EXPONENT_BITS_P_25_07;
    for (uint8_t i = SIGNIFICAND_BITS_P_25_07 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_64(uint64_t n) {
    ...
    uint64_t copy = n << (2 * EXPONENT_BITS_P_25_07);
    if (copy == 0U) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_25_07 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0U) {
            return i;
        }
...
p2507_t new_p2507(int32_t exp, int32_t sig) {
    return (exp << SIGNIFICAND_BITS_P_25_07) | (sig & SIGNIFICAND_MASK_P_25_07);
}

// exponent part of p-adic 2507
int32_t exp_p2507(p2507_t a) {
    uint32_t exp_a = (a & EXPONENT_MASK_P_25_07) >> SIGNIFICAND_BITS_P_25_07;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_P_25_07 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_25_07 << EXPONENT_BITS_P_25_07; // Fill the left with ones
    }
    return exp_a;
}
...
int32_t sig_p2507(p2507_t a) {
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_25_07;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_P_25_07 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_25_07;
    }
    return sig_a;
}
...
p2507_t add_p2507(p2507_t a, p2507_t b) {
    ...
    int32_t exp_a = (a & EXPONENT_MASK_P_25_07) >> SIGNIFICAND_BITS_P_25_07;
    int32_t exp_b = (b & EXPONENT_MASK_P_25_07) >> SIGNIFICAND_BITS_P_25_07;
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_25_07;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_25_07;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_25_07 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_25_07;
    ...
    if ((EXPONENT_SIGN_MASK_P_25_07 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_25_07 << EXPONENT_BITS_P_25_07; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p2507(result_exp, result_sig);
}

p2507_t sub_p2507(p2507_t a, p2507_t b) {
    return add_p2507(a, neg_p2507(b));
}

p2507_t neg_p2507(p2507_t b) {
    // Negate b and add
    uint32_t exp_b = b & EXPONENT_MASK_P_25_07;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_25_07;

    // Flip all bits of significand for two's complement
    if ((1ULL << (SIGNIFICAND_BITS_P_25_07 - 1) & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_25_07;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_25_07;
    }
...
p2507_t mul_p2507(p2507_t a, p2507_t b) {
    uint32_t exp_a = (a & EXPONENT_MASK_P_25_07) >> SIGNIFICAND_BITS_P_25_07;
    uint32_t exp_b = (b & EXPONENT_MASK_P_25_07) >> SIGNIFICAND_BITS_P_25_07;
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_25_07;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_25_07;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_25_07 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_25_07;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_25_07;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_25_07 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_25_07;
    ...
    // Normalize result
    uint32_t result_sig = (uint32_t)temp_sig & SIGNIFICAND_MASK_P_25_07;
    if (msb > 0) {
...
p2507_t div_p2507(p2507_t a, p2507_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_P_25_07;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_P_25_07;
        }
    }
    ...
    uint32_t mask_r = 1U;
    let((sig_b | 1U) != 0U);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_25_07; i++) {
        uint32_t mask_j = 2U;
        if (i > 0) {
            mask_r = 1U << (i - 1);
        }
...
int com_p2507(p2507_t a, p2507_t b) {
    ...
    int32_t exp_a = (a & EXPONENT_MASK_P_25_07) >> SIGNIFICAND_BITS_P_25_07;
    int32_t exp_b = (b & EXPONENT_MASK_P_25_07) >> SIGNIFICAND_BITS_P_25_07;
    int32_t sig_a = a & SIGNIFICAND_MASK_P_25_07;
    int32_t sig_b = b & SIGNIFICAND_MASK_P_25_07;
