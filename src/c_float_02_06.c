...
static uint8_t get_msb_index_positive(uint8_t n) {
    ...
    uint8_t copy = n & SIGNIFICAND_MASK_F_02_06;
    for (uint8_t i = 1; i <= SIGNIFICAND_BITS_F_02_06; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_16(uint16_t n) {
    ...
    uint16_t copy = n & SIGNIFICAND_2_MASK_F_02_06;
    for (uint8_t i = 1; i <= 2 * SIGNIFICAND_BITS_F_02_06; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
f0206_t new_f0206(int8_t exp, int8_t sig) {
    return (exp << SIGNIFICAND_BITS_F_02_06) | (sig & SIGNIFICAND_MASK_F_02_06);
}

// exponent part of float 0206
int8_t exp_f0206(f0206_t a) {
    uint8_t exp_a = (a & EXPONENT_MASK_F_02_06) >> SIGNIFICAND_BITS_F_02_06;

    // Handle negatives exponents
    if (EXPONENT_SIGN_MASK_F_02_06 & exp_a) {
        exp_a |= SIGNIFICAND_MASK_F_02_06 << EXPONENT_BITS_F_02_06; // Fill the left with ones
    }
    return exp_a;
}

// significand part of float 0206
int8_t sig_f0206(f0206_t a) {
    uint8_t sig_a = a & SIGNIFICAND_MASK_F_02_06;

    // Handle negative numbers
    if (SIGNIFICAND_SIGN_MASK_F_02_06 & sig_a) {
        sig_a |= EXPONENT_MASK_F_02_06;
    }
    return sig_a;
}
...
f0206_t add_f0206(f0206_t a, f0206_t b) {
    ...
    int8_t exp_a = (a & EXPONENT_MASK_F_02_06) >> SIGNIFICAND_BITS_F_02_06;
    int8_t exp_b = (b & EXPONENT_MASK_F_02_06) >> SIGNIFICAND_BITS_F_02_06;
    uint8_t sig_a = a & SIGNIFICAND_MASK_F_02_06;
    uint8_t sig_b = b & SIGNIFICAND_MASK_F_02_06;
    ...
    if (SIGNIFICAND_SIGN_MASK_F_02_06 & sig_b) {
        sig_b |= EXPONENT_MASK_F_02_06;
        posi_b = false;
    ...
    if (EXPONENT_SIGN_MASK_F_02_06 & exp_b) {
        exp_b |= SIGNIFICAND_MASK_F_02_06 << EXPONENT_BITS_F_02_06; // Fill the left with ones
    }
    ...
    if (exp_a > exp_b) {
        ...
        sig_b >>= diff;
        if (!posi_b) {
            sig_b |= ((1U << diff) - 1) << (BITS_F_02_06 - diff); // Fill the left with ones
            if ((sig_b & SIGNIFICAND_MASK_F_02_06) == SIGNIFICAND_MASK_F_02_06) {
                sig_b = 0;
            }
        }
        ...
        sig_a >>= diff;
        if (!posi_a) {
            sig_a |= ((1U << diff) - 1) << (BITS_F_02_06 - diff); // Fill the left with ones
            if ((sig_a & SIGNIFICAND_MASK_F_02_06) == SIGNIFICAND_MASK_F_02_06) {
                sig_a = 0;
            }
        }
    ...
    if (BITS_SIGN_MASK_F_02_06 & result_sig) {
       posi_result = false;
    }
    ...
    msb -= SIGNIFICAND_BITS_F_02_06 - 1;
    ...
    // Assemble result
    return new_f0206(result_exp, result_sig);
}

f0206_t sub_f0206(f0206_t a, f0206_t b) {
    return add_f0206(a, neg_f0206(b));
}

f0206_t neg_f0206(f0206_t b) {
    // Negate b and add
    uint8_t exp_b = b & EXPONENT_MASK_F_02_06;
    uint8_t sig_b = b & SIGNIFICAND_MASK_F_02_06;

    // Flip all bits of significand for two's complement
    if ((1ULL << (SIGNIFICAND_BITS_F_02_06 - 1) & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_F_02_06;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_F_02_06;
    }
...
f0206_t mul_f0206(f0206_t a, f0206_t b) {
    uint8_t exp_a = (a & EXPONENT_MASK_F_02_06) >> SIGNIFICAND_BITS_F_02_06;
    uint8_t exp_b = (b & EXPONENT_MASK_F_02_06) >> SIGNIFICAND_BITS_F_02_06;
    uint8_t sig_a = a & SIGNIFICAND_MASK_F_02_06;
    uint8_t sig_b = b & SIGNIFICAND_MASK_F_02_06;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if (SIGNIFICAND_SIGN_MASK_F_02_06 & sig_a) {
        sig_a |= EXPONENT_MASK_F_02_06;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_F_02_06;
        log_debug_label("a is negative");
    }
    if (SIGNIFICAND_SIGN_MASK_F_02_06 & sig_b) {
        sig_b |= EXPONENT_MASK_F_02_06;
    ...
    // Normalize result
    uint8_t result_sig = (uint8_t)temp_sig & SIGNIFICAND_MASK_F_02_06;
    if (msb > 0) {
    ...
    result_exp += msb - SIGNIFICAND_BITS_F_02_06 + 1;
    log_debug_float("result significand", result_sig);
    log_debug_float("result exp", result_exp);
...
f0206_t div_f0206(f0206_t a, f0206_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_F_02_06;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_F_02_06;
        }
    }

    // Perform division
    uint16_t temp_sig = ((uint16_t)sig_a << SIGNIFICAND_BITS_F_02_06) / sig_b;
...
int com_f0206(f0206_t a, f0206_t b) {
    ...
    int8_t exp_a = (a & EXPONENT_MASK_F_02_06) >> SIGNIFICAND_BITS_F_02_06;
    int8_t exp_b = (b & EXPONENT_MASK_F_02_06) >> SIGNIFICAND_BITS_F_02_06;
    int8_t sig_a = a & SIGNIFICAND_MASK_F_02_06;
    int8_t sig_b = b & SIGNIFICAND_MASK_F_02_06;
