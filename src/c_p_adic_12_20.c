#include "c_p_adic_12_20.h"
#include <assert.h>
#include <stdint.h>
#include <stdio.h>

static void log_debug_label(const char* label) {
    if (C_P_ADIC_12_20_DEBUG) {
        printf("%s\n", label);
    }
}
...
static uint8_t get_msb_index(uint32_t n) {
    if (n == 0) return SIGNIFICAND_BITS_P_12_20 - 1; // Handle 0 as a special case

    uint32_t copy = n << EXPONENT_BITS_P_12_20;
    for (uint8_t i = SIGNIFICAND_BITS_P_12_20 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_64(uint64_t n) {
    ...
    uint64_t copy = n << (2 * EXPONENT_BITS_P_12_20);
    if (copy == 0U) return 0; // Handle 0 as a special case
    for (uint8_t i = 2 * SIGNIFICAND_BITS_P_12_20 - 1; i >= 0; i--) {
        copy <<= 1;
        if (copy == 0U) {
            return i;
        }
...
p1220_t new_p1220(int32_t exp, int32_t sig) {
    return (exp << SIGNIFICAND_BITS_P_12_20) | (sig & SIGNIFICAND_MASK_P_12_20);
}

// exponent part of p-adic 1220
int32_t exp_p1220(p1220_t a) {
    uint32_t exp_a = (a & EXPONENT_MASK_P_12_20) >> SIGNIFICAND_BITS_P_12_20;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_P_12_20 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_P_12_20 << EXPONENT_BITS_P_12_20; // Fill the left with ones
    }
    return exp_a;
}

// significand part of p-adic 1220
int32_t sig_p1220(p1220_t a) {
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_12_20;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_P_12_20 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_12_20;
    }
    return sig_a;
}
...
p1220_t add_p1220(p1220_t a, p1220_t b) {
    ...
    int32_t exp_a = (a & EXPONENT_MASK_P_12_20) >> SIGNIFICAND_BITS_P_12_20;
    int32_t exp_b = (b & EXPONENT_MASK_P_12_20) >> SIGNIFICAND_BITS_P_12_20;
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_12_20;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_12_20;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_12_20 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_12_20;
    ...
    if ((EXPONENT_SIGN_MASK_P_12_20 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_P_12_20 << EXPONENT_BITS_P_12_20; // Fill the left with ones
    }
    ...
    // Assemble result
    return new_p1220(result_exp, result_sig);
}

p1220_t sub_p1220(p1220_t a, p1220_t b) {
    return add_p1220(a, neg_p1220(b));
}

p1220_t neg_p1220(p1220_t b) {
    // Negate b and add
    uint32_t exp_b = b & EXPONENT_MASK_P_12_20;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_12_20;

    // Flip all bits of significand for two's complement
    if ((1ULL << (SIGNIFICAND_BITS_P_12_20 - 1) & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_P_12_20;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_P_12_20;
    }
...
p1220_t mul_p1220(p1220_t a, p1220_t b) {
    uint32_t exp_a = (a & EXPONENT_MASK_P_12_20) >> SIGNIFICAND_BITS_P_12_20;
    uint32_t exp_b = (b & EXPONENT_MASK_P_12_20) >> SIGNIFICAND_BITS_P_12_20;
    uint32_t sig_a = a & SIGNIFICAND_MASK_P_12_20;
    uint32_t sig_b = b & SIGNIFICAND_MASK_P_12_20;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_P_12_20 & sig_a)) {
        sig_a |= EXPONENT_MASK_P_12_20;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_P_12_20;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_P_12_20 & sig_b)) {
        sig_b |= EXPONENT_MASK_P_12_20;
    ...
    // Normalize result
    uint32_t result_sig = (uint32_t)temp_sig & SIGNIFICAND_MASK_P_12_20;
    if (msb > 0) {
...
p1220_t div_p1220(p1220_t a, p1220_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_P_12_20;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_P_12_20;
        }
    }
    ...
    uint32_t mask_r = 1U;
    let((sig_b | 1U) != 0U);
    for (int8_t i = 0; i < SIGNIFICAND_BITS_P_12_20; i++) {
        uint32_t mask_j = 2U;
        if (i > 0) {
            mask_r = 1U << (i - 1);
        }
...
int com_p1220(p1220_t a, p1220_t b) {
    ...
    int32_t exp_a = (a & EXPONENT_MASK_P_12_20) >> SIGNIFICAND_BITS_P_12_20;
    int32_t exp_b = (b & EXPONENT_MASK_P_12_20) >> SIGNIFICAND_BITS_P_12_20;
    int32_t sig_a = a & SIGNIFICAND_MASK_P_12_20;
    int32_t sig_b = b & SIGNIFICAND_MASK_P_12_20;
