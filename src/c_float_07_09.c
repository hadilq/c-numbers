...
static uint8_t get_msb_index_positive(uint16_t n) {
    ...
    uint16_t copy = n & SIGNIFICAND_MASK_F_07_09;
    for (uint8_t i = 1; i <= SIGNIFICAND_BITS_F_07_09; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
static uint8_t get_msb_index_32(uint32_t n) {
    ...
    uint32_t copy = n & SIGNIFICAND_2_MASK_F_07_09;
    for (uint8_t i = 1; i <= 2 * SIGNIFICAND_BITS_F_07_09; i++) {
        copy >>= 1;
        if (copy == 0) {
            return i;
        }
...
f0709_t new_f0709(int16_t exp, int16_t sig) {
    return (exp << SIGNIFICAND_BITS_F_07_09) | (sig & SIGNIFICAND_MASK_F_07_09);
}
...
f0709_t new_f0709_from_float16(_Float16 f) {
    ...
    if (SIGNIFICAND_BITS_F_07_09 > Float16_SIG_1) {
        significand <<= SIGNIFICAND_BITS_F_07_09 - Float16_SIG_1;
    } else {
        significand >>= Float16_SIG_1 - SIGNIFICAND_BITS_F_07_09;
    }
    return new_f0709(exponent, significand);
}

// exponent part of float 0709
int16_t exp_f0709(f0709_t a) {
    uint16_t exp_a = (a & EXPONENT_MASK_F_07_09) >> SIGNIFICAND_BITS_F_07_09;

    // Handle negatives exponents
    if ((EXPONENT_SIGN_MASK_F_07_09 & exp_a)) {
        exp_a |= SIGNIFICAND_MASK_F_07_09 << EXPONENT_BITS_F_07_09; // Fill the left with ones
    }
    return exp_a;
}
...
int16_t sig_f0709(f0709_t a) {
    uint16_t sig_a = a & SIGNIFICAND_MASK_F_07_09;

    // Handle negative numbers
    if ((SIGNIFICAND_SIGN_MASK_F_07_09 & sig_a)) {
        sig_a |= EXPONENT_MASK_F_07_09;
    }
    return sig_a;
}
...
f0709_t add_f0709(f0709_t a, f0709_t b) {
    ...
    int16_t exp_a = (a & EXPONENT_MASK_F_07_09) >> SIGNIFICAND_BITS_F_07_09;
    int16_t exp_b = (b & EXPONENT_MASK_F_07_09) >> SIGNIFICAND_BITS_F_07_09;
    uint16_t sig_a = a & SIGNIFICAND_MASK_F_07_09;
    uint16_t sig_b = b & SIGNIFICAND_MASK_F_07_09;
    ...
    if ((SIGNIFICAND_SIGN_MASK_F_07_09 & sig_b)) {
        sig_b |= EXPONENT_MASK_F_07_09;
        posi_b = false;
    ...
    if ((EXPONENT_SIGN_MASK_F_07_09 & exp_b)) {
        exp_b |= SIGNIFICAND_MASK_F_07_09 << EXPONENT_BITS_F_07_09; // Fill the left with ones
    }
    ...
    if (exp_a > exp_b) {
        ...
        sig_b >>= diff;
        if (!posi_b) {
            sig_b |= ((1U << diff) - 1) << (BITS_F_07_09 - diff); // Fill the left with ones
            if ((sig_b & SIGNIFICAND_MASK_F_07_09) == SIGNIFICAND_MASK_F_07_09) {
                sig_b = 0;
            }
        }
        ...
        sig_a >>= diff;
        if (!posi_a) {
            sig_a |= ((1U << diff) - 1) << (BITS_F_07_09 - diff); // Fill the left with ones
            if ((sig_a & SIGNIFICAND_MASK_F_07_09) == SIGNIFICAND_MASK_F_07_09) {
                sig_a = 0;
            }
        }
    ...
    if (BITS_SIGN_MASK_F_07_09 & result_sig) {
       posi_result = false;
    }
    ...
    msb -= SIGNIFICAND_BITS_F_07_09 - 1;
    log_debug_float("msb", msb);
    ...
    // Assemble result
    return new_f0709(result_exp, result_sig);
}

f0709_t sub_f0709(f0709_t a, f0709_t b) {
    return add_f0709(a, neg_f0709(b));
}

f0709_t neg_f0709(f0709_t b) {
    // Negate b and add
    uint16_t exp_b = b & EXPONENT_MASK_F_07_09;
    uint16_t sig_b = b & SIGNIFICAND_MASK_F_07_09;

    // Flip all bits of significand for two's complement
    if ((SIGNIFICAND_SIGN_MASK_F_07_09 & sig_b)) {
        // negative
        sig_b = (~sig_b + 1) & SIGNIFICAND_MASK_F_07_09;
    } else {
        sig_b = (~(sig_b - 1)) & SIGNIFICAND_MASK_F_07_09;
    }
...
f0709_t mul_f0709(f0709_t a, f0709_t b) {
    uint16_t exp_a = (a & EXPONENT_MASK_F_07_09) >> SIGNIFICAND_BITS_F_07_09;
    uint16_t exp_b = (b & EXPONENT_MASK_F_07_09) >> SIGNIFICAND_BITS_F_07_09;
    uint16_t sig_a = a & SIGNIFICAND_MASK_F_07_09;
    uint16_t sig_b = b & SIGNIFICAND_MASK_F_07_09;
    bool posi_a = true;
    bool posi_b = true;
    ...
    if ((SIGNIFICAND_SIGN_MASK_F_07_09 & sig_a)) {
        sig_a |= EXPONENT_MASK_F_07_09;
        posi_a = false;
        sig_a = (~(sig_a - 1)) & SIGNIFICAND_MASK_F_07_09;
        log_debug_label("a is negative");
    }
    if ((SIGNIFICAND_SIGN_MASK_F_07_09 & sig_b)) {
        sig_b |= EXPONENT_MASK_F_07_09;
    ...
    // Normalize result
    uint16_t result_sig = (uint16_t)temp_sig & SIGNIFICAND_MASK_F_07_09;
    if (msb > 0) {
        result_sig = temp_sig >> msb;
    } else if (msb < 0) {
        result_sig = temp_sig << abs(msb);
    }
    result_exp += msb - SIGNIFICAND_BITS_F_07_09 + 1;
    log_debug_float("result significand", result_sig);
...
f0709_t div_f0709(f0709_t a, f0709_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_F_07_09;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_F_07_09;
        }
    }

    // Perform division
    uint32_t temp_sig = ((uint32_t)sig_a << SIGNIFICAND_BITS_F_07_09) / sig_b;
...
int com_f0709(f0709_t a, f0709_t b) {
    ...
    int16_t exp_a = (a & EXPONENT_MASK_F_07_09) >> SIGNIFICAND_BITS_F_07_09;
    int16_t exp_b = (b & EXPONENT_MASK_F_07_09) >> SIGNIFICAND_BITS_F_07_09;
    int16_t sig_a = a & SIGNIFICAND_MASK_F_07_09;
    int16_t sig_b = b & SIGNIFICAND_MASK_F_07_09;
