...
fDDD32_t new_fDDD32_from_float32(_Float32 f) {
    ...
    if (SIGNIFICAND_BITS_F_CCC_32 > Float32_SIG_1) {
        significand <<= SIGNIFICAND_BITS_F_CCC_32 - Float32_SIG_1;
    } else {
        significand >>= Float32_SIG_1 - SIGNIFICAND_BITS_F_CCC_32;
    }
    return new_fDDD32(exponent, significand);
}
  #endif
#endif
...
fDDD32_t new_fDDD32_from_float16(_Float16 f) {
    ...
    if (SIGNIFICAND_BITS_F_CCC_32 > Float16_SIG_1) {
        significand <<= SIGNIFICAND_BITS_F_CCC_32 - Float16_SIG_1;
    } else {
        significand >>= Float16_SIG_1 - SIGNIFICAND_BITS_F_CCC_32;
    }
    return new_fDDD32(exponent, significand);
}
  #endif
...
fDDD32_t div_fDDD32(fDDD32_t a, fDDD32_t b) {
    ...
    if (sig_b == 0U) {
        if (posi_a) {
            ...
            return MAX_VALUE_F_CCC_32;
        } else {
            // Return min value as "-infinity"
            return MIN_VALUE_F_CCC_32;
        }
    }
